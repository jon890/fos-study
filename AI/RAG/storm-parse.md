...
- STORM Parse는 위와 같은 문제들을 해결하기 위해 Vision-Language Model을 핵심 기술로 채택했다.
  - VLM은 이미지와 텍스트를 동시에 처리하는 멀티모달 모델로, GPT·Claude·Gemini 계열이 여기 해당한다.
  - VLM이 이미지를 어떻게 읽는지는 [멀티모달 LLM](../llm/multimodal.md)에서 더 다룬다.
