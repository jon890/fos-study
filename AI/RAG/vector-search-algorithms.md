...
> **코사인은 사실 내적으로 계산된다.** `cos = (A·B) / (|A|·|B|)` 인데, 저장할 때 벡터를 L2 정규화(크기를 1로)하면 `|A|=|B|=1` 이라 코사인이 그냥 내적(A·B)이 된다. 그래서 대부분의 벡터 DB가 정규화 후 빠른 내적으로 코사인을 처리한다(OpenSearch `cosinesimil`, faiss cosine). HNSW는 유사도가 아니라 거리로 다루므로 `1 − 코사인`으로 변환해 쓴다.
...
```mermaid
flowchart TB
    Q[쿼리 진입] --> L2[Layer 2<br/>희소 · 멀리 점프]
    L2 --> L1[Layer 1<br/>중간]
    L1 --> L0[Layer 0<br/>전체 노드 · 정밀 탐색]
    L0 --> R[최근접 k개]
```
...
> 파라미터 상호작용(M↔ef_construction), 구현체별(FAISS·Lucene·Qdrant·pgvector·Milvus) 성능 차이, 필터링 충돌 같은 심화 주제는 [HNSW 심화 — 파라미터 튜닝과 구현체별 성능 차이](./hnsw-deep-dive.md)에서 다룬다.
