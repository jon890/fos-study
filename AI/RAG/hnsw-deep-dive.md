...
HNSW 는 여러 층의 그래프다. 위층은 노드가 희소해 큰 도약으로 대략 위치를 잡고, 내려갈수록 조밀해져 정밀하게 좁힌다.

```mermaid
flowchart TB
  q["query 진입"] --> L2["Layer 2 (희소)<br/>큰 도약으로 대략 위치"]
  L2 -->|"하강"| L1["Layer 1<br/>더 조밀, 근처로 이동"]
  L1 -->|"하강"| L0["Layer 0 (전체 노드)<br/>ef_search 만큼 후보 정밀 탐색"]
  L0 --> top["top-k 최근접"]
```

`ef_search` 는 Layer 0 에서 몇 개의 후보를 유지하며 탐색할지를 정하는 손잡이다 — 크게 잡으면 recall 이 오르고 QPS 가 떨어진다.
