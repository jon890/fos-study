...
- **새로운 AI 코딩 도구들이 나올 때마다, 새로 설치하고 공부하기 귀찮았다.**
  ...
  - 오픈소스로 관리되는, 여러 명이 기여할 수 있는 제품을 사용하는게 더 좋지 않을까?
...
- **기존 Claude 경험을 그대로 느낄 수 있는 plugin이 존재한다**
  ...
  - 이 부분은 plugin을 통해 해결이 가능하다. `oh-my-opencode`라는 플러그인이고 한국인이 만들었다.
    - 일단 Claude 커맨드나 스킬을 그대로 사용할 수 있고, 다른 여러가지 기능들을 제공한다. 이 부분은 뒤에서 더 설명해보도록 하겠다.
