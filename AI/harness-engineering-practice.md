...
## 메타 피드백 루프 — 한 번 잡고 끝이 아니다

`build-with-teams`로 한 plan을 굴리는 동안 critic ↔ team-lead 사이에 짧은 피드백이 돈다. 이걸 1차 루프라고 부르자. 글이 길어지면서 그 바깥에 두 개의 루프가 더 있다는 게 보였다. 스킬이 자기 자신을 개정하는 2차 루프, 외부 도구의 마찰을 다시 돌려보내는 3차 루프.

### 1차 루프: critic ↔ team-lead (단일 plan 안)

이미 다뤘다. 한 plan 안에서 critic이 REVISE를 내면 team-lead가 task 파일을 고치고 다시 제출한다. 이 루프는 짧고 닫혀 있다.

### 2차 루프: critic 패턴 → common-pitfalls.md → 다음 plan 사전 소진

여기서 흥미로운 게 일어난다. critic이 같은 패턴의 지적을 여러 plan에 걸쳐 반복하기 시작했다.

- "phase 작업 항목 5개 초과 — 분리 필요"
- "성공 기준이 '눈으로 확인'이라 검증 불가"
- "외부 push/머지 동작 앞에 상태 확인 게이트가 없음"

처음 한두 번은 plan을 고쳐서 통과시켰다. 그런데 다섯 번째 같은 지적을 받았을 때 깨달았다. **이건 plan을 고치는 게 아니라 스킬을 고쳐야 한다.**

`_shared/common-pitfalls.md` 파일을 만들고 critic이 반복 지적한 패턴을 항목으로 누적했다. `/planning` 스킬에는 task 파일을 사용자에게 제출하기 전에 이 파일을 열어 8개 패턴을 self-check하도록 절차를 박아 넣었다.

이렇게 하면 critic이 같은 지적을 더 이상 안 한다. critic이 봐야 하는 패턴은 새로운 것 — 코드베이스 변경에 따라 처음 등장한 가정 불일치 — 로 좁혀진다. critic의 시간이 진짜 가치 있는 곳에 쓰이고, 사이클이 짧아진다.

이게 2차 루프다. **사고 → 패턴화 → 스킬 자가-개정 → 다음 사이클 사전 소진.**

다른 도메인에서도 같은 일이 일어난다. OCR API 백엔드 작업기를 블로그로 정리하다가 마크다운 함정에 한 번 빠졌다. GitHub Flavored Markdown은 한 paragraph에 `~`가 두 번 이상 들어가면 두 `~` 사이를 취소선으로 렌더링한다. 범위 표기를 자주 쓰는 기술 글에서 무심코 발동되기 쉽다 (`30분\~1시간`, `60\~100건` 같이 한 문단에 같이 등장하는 케이스). 한 번 사고 후에는 글 작성 스킬에 "본문 작성 후 `grep -n` 으로 paragraph당 등장 횟수를 자가 점검하라"는 절차를 박았다. 같은 자리에 `§` 문자 사용 금지 룰도 누적됐다. 사소한 마이크로 함정도 다음 사이클부터는 자동 점검된다.

### 3차 루프: 외부 도구 마찰 → CLI 개선 요청 → 다음 사이클

스킬이 자가 개정하는 루프 위에 한 단계 더 있다. **스킬도 못 고치는 외부 도구의 마찰**을 다루는 루프.

이번 OCR 백엔드 사이클에서 두 가지 마찰을 만났다.

첫째, 사내 이슈 트래커 CLI의 `--json` 출력에 progress 메시지가 stdout으로 섞여 나왔다. 자동화 스크립트마다 `sed`로 첫 줄을 걸러내고 `jq`로 넘기는 우회가 반복됐다. 이건 내가 짠 스킬 안에 박아 넣을 문제가 아니다. 매 자동화에서 같은 우회를 박는 건 외부 도구가 책임져야 한다.

둘째, 댓글 본문 edit이 patch가 아니라 full replace라서 새 본문에 inline 이미지 markdown reference를 빠뜨리면 이미지가 안 보였다. attachment 자체는 댓글에 그대로 살아있는데 본문에서만 reference가 빠진 상태였다. 한 번 부딪힌 뒤 raw API로 file ID 보존이 가능한 것을 확인했지만, CLI 사용자가 매번 의식해야 하는 사용성 문제다.

두 마찰 모두 외부 저장소에 GitHub issue로 정리해 등록했다. 자동화 사용성 4종 묶음 하나, 댓글 첨부파일 명령 추가 하나.

이게 3차 루프다. **스킬에 우회를 박아 넣지 않고, 외부 도구의 정책 자체를 바꾸도록 feedback을 돌려보낸다.** 그러면 다음 사이클에서 그 마찰 자체가 사라진다 — 또는 적어도 후순위로 밀린다. 우회를 코드화하면 이쪽 시스템이 무거워지지만, 마찰을 외부로 돌려보내면 양쪽이 같이 가벼워진다.

### 다층 루프

```mermaid
flowchart TD
    subgraph L1["1차 루프 (분~시간)"]
        TL[team-lead]
        CR[critic]
        TL -->|task 제출| CR
        CR -->|REVISE/APPROVE| TL
    end

    subgraph L2["2차 루프 (일~주)"]
        PAT[critic 반복 패턴]
        PIT[common-pitfalls.md]
        SC[스킬 self-check 절차]
        PAT --> PIT --> SC
    end

    subgraph L3["3차 루프 (주~월)"]
        FRC[CLI/repo 정책 마찰]
        ISS[GitHub issue 등록]
        TOOL[외부 도구 개선]
        FRC --> ISS --> TOOL
...
각 루프의 시간 단위가 다르다. **루프가 외곽일수록 효과가 영구적이다.** 1차 루프는 plan 하나만 고치고 끝나지만, 2차 루프는 모든 다음 plan에 작용하고, 3차 루프는 모든 다음 자동화에 작용한다.

하네스가 잘 작동한다는 건 1차 루프가 잘 도는 것만 의미하지 않는다. 1차에서 잡힌 패턴이 2차로 흘러가고, 2차에서 막히는 도구 마찰이 3차로 흘러가야 한다. 사고 한 번이 시스템 안 어딘가에 영구 흔적을 남기는 구조 — 그게 진화하는 하네스다.
...
여기서 한 단계 더 들어간 게 메타 피드백 루프다. 한 plan 안에서 잡힌 패턴은 1차 루프로 끝나는 게 아니라 스킬 자가 개정으로 흘러가고, 스킬도 못 고치는 외부 도구 마찰은 issue로 외부에 돌려보낸다. 사고 하나가 시스템 안 어딘가에 영구 흔적을 남길 수 있어야 진짜 하네스가 진화한다.

"The model is commodity. The harness is moat." — 이 말이 진짜로 이해된 건 몇 번의 실패를 구조로 바꾸고 나서였다. 그리고 그 구조 자체가 다시 진화하는 사이클을 갖췄을 때다.
