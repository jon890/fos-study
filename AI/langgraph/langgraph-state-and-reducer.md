...
이 글의 개념은 작은 상태 스키마 하나를 직접 설계해보면 빨리 선명해진다.
완성 코드를 복사하지 말고 다음 순서만 따라간다.

1. 개발 작업 요청을 나타내는 `request`와 검토 결과 목록 `findings`를 상태 키로 정한다.
2. `request`는 새 입력이 오면 덮어쓰고, `findings`는 여러 노드의 결과를 누적한다고 가정한다.
3. 두 키에 같은 병합 규칙을 쓰면 어느 쪽이 깨지는지 먼저 예상한다.
