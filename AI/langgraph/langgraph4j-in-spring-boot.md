...
이 글의 코드를 한 번에 복사하기보다 학습 저장소를 옆에 띄우고 작은 단위로 작성하는 편이 낫다.

```bash
git clone https://github.com/jon890/langgraph4j-in-action.git
cd langgraph4j-in-action
./gradlew test
```
...
Ollama나 무료 API는 결정적인 그래프가 먼저 동작한 뒤 연결한다.
