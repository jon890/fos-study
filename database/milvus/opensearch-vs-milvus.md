...
```mermaid
flowchart TB
    subgraph OS[OpenSearch · 결합형]
        OC[Coordinating Node] --> OD[Data Node<br/>샤드 + 벡터 메모리]
        OM[Master Node · 메타]
    end
    subgraph MV[Milvus · 분리형]
        MP[Proxy] --> MQ[Query Node<br/>벡터 메모리]
        MCO[Coordinator · 메타]
        MQ --> MOBS[(Object Storage<br/>벡터 영속)]
    end
```
...
두 분석기의 기능 동등성·튜닝 옵션·버전별 차이는 [한국어 형태소 분석기 Nori vs Lindera](../nori-vs-lindera-korean-tokenizer.md)에서 자세히 다룬다.
