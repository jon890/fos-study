...
- 캐시 효율 저하 — 많은 커넥션이 서로 다른 데이터를 읽어 버퍼 캐시가 빠르게 교체되고 디스크 I/O가 늘어난다.
...
Oracle 의 Real-World Performance 그룹은 더 보수적으로 CPU 코어당 10 이하를 권한다. 이때도 스레드 수가 아니라 코어 수가 기준이다. PostgreSQL 공식이 하한이라면 이쪽은 상한 가이드로 보면 된다.
...
- 커넥션을 늘릴수록 느려지는 건 USL — 처리량이 꺾이는 saturation point 가 시스템 한계다.
