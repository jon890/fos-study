...
- 메시지를 하나도 놓치면 안 된다 → **Stream**
- Kafka가 과하다 싶은 경량 이벤트 큐 → **Stream**

**Stream이 "정확히 한 번"을 보장하지는 않는다.**
위 표대로 Stream의 전달 보장은 at-least-once다.
ACK 전에 소비자가 죽으면 그 메시지는 PEL(Pending Entries List)에 남고,
`XCLAIM`으로 다른 소비자에게 넘어가 **다시 처리된다.**

- 즉 "놓치지 않음"은 보장되지만 "중복되지 않음"은 보장되지 않는다
- 따라서 소비자 로직은 멱등해야 한다 — 같은 메시지를 두 번 처리해도 결과가 같아야 한다
- 멱등성 확보 수단은 메시지 ID를 처리 이력 테이블에 기록하거나,
  결과를 덮어쓰는 연산(UPSERT)으로 설계하는 방식이 흔하다
...
위 처리량은 **자릿수 감각을 잡기 위한 대략치**다.
메시지 크기, 파티션·소비자 수, 디스크와 네트워크, 배치 설정에 따라 크게 달라지므로
도입 판단은 이 표가 아니라 본인 환경의 측정값으로 해야 한다.
