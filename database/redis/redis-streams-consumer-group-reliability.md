...
# Redis Streams 소비자 그룹 신뢰성: PEL, 재할당과 멱등성
...
여기서 가장 자주 틀리는 부분이 있다. `XACK`를 호출하기 전까지 메시지는 PEL에 영원히 남는다. 소비자가 처리 도중 죽어도 메시지는 사라지지 않는다. 다시 살아난 소비자는 `>` 대신 자기 ID 범위를 지정해 자기 PEL을 다시 읽을 수 있다.
...
소비자 하나가 영영 돌아오지 않으면, 그 소비자의 PEL에 갇힌 메시지를 다른 소비자가 가져와야 한다. 이를 자동으로 해주는 장치는 없으므로 **직접 구현해야 하는 운영 책임**이다.
...
### XAUTOCLAIM으로 스캔과 클레임을 한 번에 처리한다
