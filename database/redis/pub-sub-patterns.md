...
flowchart TD
    A["Admin API"]
    DB["DB"]
    P["PUBLISH cache.invalidate 'policy:menu:1001'"]
    App1["App-1<br/>L1 evict"]
    App2["App-2<br/>L1 evict"]
    AppN["App-N<br/>L1 evict"]

    A -->|"① 정책 변경 (DB write)"| DB
    DB -->|"② AFTER_COMMIT 이벤트"| P
    P --> App1
    P --> App2
    P --> AppN
```
...
flowchart TD
    L["Logout API"]
    RS["Redis SADD revoked:tokens &lt;jti&gt; EX 3600"]
    P["PUBLISH session.revoke &lt;jti&gt;"]
    App1["App-1 L1<br/>blacklist update"]
    App2["App-2 L1<br/>blacklist update"]
    AppN["App-N L1<br/>blacklist update"]
