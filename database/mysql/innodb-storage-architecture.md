...
MySQL 5.7 버전을 거쳐서 MySQL 8.0 버전으로 업그레이드되면서 대부분의 서비스에서는 더티 페이지를 디스크에 동기화하는 부분에서 예전과 같은 디스크 쓰기 폭증 현상은 발생하지 않는다.
특별히 서비스를 운영할 때 성능 문제가 발생하지 않는 상태라면 시스템 변수를 조정할 일은 없을 것이다.

InnoDB 엔진은 버퍼 풀에서 아직 디스크로 기록되지 않은 더티 페이지들을 성능상의 악영향 없이 디스크에 동기화 하기 위해 다음과 같이 2개의 플러시 기능을 백그라운드로 실행한다.

- 플러시 리스트 (Flust_list) 플러시
- LRU 리스트 (LRU_list) 플러시

##### 플러시 리스트 플러시

InnoDB 스토리지 엔진은 리두 공간의 재활용을 위해 주기적으로 오래된 리두 로그 엔트리가 사용하는 공간을 비워야 한다.
그런데 오래된 리두 공간이 지워지려면 반드시 InnoDB 벌퍼 풀의 더티 페이지가 먼저 디스크로 동기화돼야 한다.

이를 위해 InnoDB 스토리지 엔진은 주기적으로 플러시 리스트(Flust_list) 플러시 함수를 호출해서 플러시 리스트에서 오래전에 변경된 데이터 페이지 순서대로 디스크에 동기화하는 작업을 수행한다.

> Flush List 란?
>
> 버퍼 풀은 페이지들이 실제로 저장되는 메모리 공간이고, Flush List는 **더티 페이지**만 따로 모아서 관리하는 리스트다. </br>
> 복사가 아니라 **참조 리스트**
>
> ```mermaid
> flowchart TB
...
>     end
>
>     Page1 -->|Dirty 되면 등록| DP1
>     Page2 -->|Dirty 되면 등록| DP2
> ```
...
>
>    alt 처음 Dirty 되는 경우
>        BP->>FL: Flush List에 등록
>    else 이미 Dirty
>        Note right of BP: Flush List 위치만 갱신
>    end
> ```
...
- `innodb_page_cleaners`
  - InnoDB 스토리지 엔진에서 더티 페이지를 동기화하는 스레드를 클리너 스레드라고 하며, 이 스레드 개수를 조정할 수 있게 한다.
  - InnoDB 스토리지 엔진은 여러 개의 버퍼 풀 인스턴스를 동시에 사용할 수 있는데, 이 설정값이 버퍼 풀 인스턴스 개수보다 많은 경우에는 `innodb_buffer_pool_instances` 설정값으로 자동으로 맞춰준다.
  - 가능하면 `innodb_page_cleaners` 설정값과 `innodb_buffer_pool_instances` 설정값을 동일한 값으로 설정하자.
- `innodb_max_dirty_pages_pct`
  - 기본적으로 InnoDB 스토리지 엔진은 전체 버퍼 풀이 가진 페이지의 90%까지 더티 페이지를 가질 수 있다.
  - 이 변수를 이용해 더티 페이지의 비율을 조정할 수 있다.
  - 일반적으로 버퍼 풀은 더티 페이지를 많이 가지고 있을수록 쓰기 작업을 버퍼링하여 디스크 쓰기를 줄일 수 있으므로 가능하면 기본값을 사용하자.
- `innodb_max_dirty_pages_pct_lwm`
  - 급작스럽게 디스크 쓰기가 폭증하는 현상이 발생할 때, 완화하기 위해 일정 수준 이상의 더티 페이지가 발생하면 조금씩 더티 페이지를 디스크로 기록하게 한다.
  - 기본값은 10% 수준이다.
  - 더티 페이지의 비율이 얼마되지 않는 상태에서 디스크 쓰기가 많이 발생하고 더티 페이지의 비율이 낮은 상태로 머물러 있다면 조금 더 높은 값으로 조정해보자.
- `innodb_io_capacity`
  - 데이터베이스 서버에서 어느 정도의 디스크 읽고 쓰기가 가능한지를 설정하는 값이다.
    - 디스크 읽고 쓰기란? InnoDB 스토리지 엔진의 백그라운드 스레드가 수행하는 디스크 작업
    - 대부분 InnoDB 버퍼 풀의 더티 페이지 쓰기가 해당함
  - 일반적인 상황에서 디스크가 적절히 처리할 수 있는 수준의 값을 설정한다.
- `innodb_io_capacity_max`
  - 디스크가 최대 성능을 발휘할 때, 어느 정도의 디스크 읽고 쓰기가 가능한지를 설정한다.
- `innodb_adaptive_flushing`
  - 관리해야할 서버가 많다면, `innodb_io_capacity`를 설정하는 것은 상당히 번거롭다.
  - 그래서 InnoDB 스토리지 엔진은 어댑티브 플러시(Adaptive Flush)라는 기능을 제공한다.
  - 어댑티브 플러시 기능을 끄거 켤 수 있는 변수이다.
  - 어댑티브 플러시 기능이란?
    - 리두 로그의 증가 속도를 분석해서 적절한 수준의 더티 페이지가 버퍼 풀에 유지될 수 있도록 디스크 쓰기를 실행한다.
- `innodb_adaptive_flushing_lwm`
  - 이 시스템 변수의 기본값은 10%이다.
  - 전체 리두 공간에서 활성 리두 공간이 10% 미만이면 어댑티브 플러시가 작동하지 않다가, 10%를 넘어서면 작동하게 한다.
- `innodb_flush_neighbors`
  - 더티 페이지를 내보낼 때 **같은 extent 안의 이웃 페이지까지 함께 기록할지**를 정한다.
  - 값은 셋이다.
    - `0` — 이웃을 함께 내보내지 않는다
    - `1` — 같은 extent 안에서 **연속된** 더티 페이지를 함께 내보낸다
    - `2` — 같은 extent 안의 더티 페이지를 함께 내보낸다
  - 기본값은 `0`이다.
  - 이웃을 묶어 내보내는 것은 디스크 헤드 이동을 아끼려는 최적화라 회전 디스크(HDD)에서 의미가 있다.
  - SSD 처럼 탐색 시간이 병목이 아닌 저장장치에서는 공식 문서가 기본값 `0`을 권장한다. 묶어봐야 아끼는 것이 없고 쓰기량만 늘기 때문이다.
  - 출처: [MySQL — Configuring Buffer Pool Flushing](https://dev.mysql.com/doc/refman/8.4/en/innodb-buffer-pool-flushing.html)
