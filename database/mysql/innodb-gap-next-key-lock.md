...
## 왜 Gap / Next-Key Lock을 따로 공부해야 하는가
...
## Next-Key Lock = Record Lock + 그 앞의 Gap
...
### 전형 데드락: "Gap Lock 두 개 + Insert Intention 두 개"
...
RECORD LOCKS space id ... index idx_user of table ...
    trx id <trx-id> lock_mode X locks gap before rec insert intention waiting
```
...
## Supremum Pseudo-Record와 무한대 gap
...
### Unique 인덱스 & 존재하는 단일 키 → Gap Lock 생략
...
### `performance_schema.data_locks` (MySQL 8)
