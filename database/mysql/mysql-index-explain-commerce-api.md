...
커머스 백엔드에서 가장 자주 마주치는 성능 문제는 거의 항상 인덱스와 실행 계획에서 출발한다. 주문 목록, 회원 쿠폰, 매장 메뉴처럼 조회 패턴이 정해져 있는 화면일수록 트래픽이 누적되고, 단일 쿼리의 비효율이 그대로 슬로우 쿼리 로그와 P99 응답 시간에 누적된다. 외식 프랜차이즈처럼 매장/회원/메뉴/주문이 얽힌 도메인에서는 한 화면에서 수십만 row를 스캔하는 비효율이 손쉽게 만들어지고, 새벽 배치/이벤트 트래픽에서 한꺼번에 터진다.

인덱스 설계가 단순 암기를 넘어서는 지점은 다음 흐름으로 문제를 보는 데 있다.
...
이 중 `type`, `key`, `Extra` 세 가지를 엮어서 읽는 능력이 실전에서 가장 자주 쓰인다.
...
정리하면 이렇다. OFFSET이 커지면 인덱스 자체는 타도 OFFSET만큼의 row를 거쳐야 해서 P99가 무너진다. cursor 기반 keyset pagination으로 바꿔서 OFFSET을 제거한다.
...
강조할 포인트는 이것이다. **"왜 인덱스를 추가하지 않았는가"도 설계 결정의 일부다.** 인덱스를 늘리면 INSERT/UPDATE/DELETE 비용이 늘고, 쓰기 트래픽이 큰 테이블에서는 락과 페이지 분할 비용도 누적된다.
...
정리하면 이렇다. driving 테이블을 먼저 식별하고, driven 쪽 인덱스를 lookup 패턴에 맞게 설계한다. JOIN 컬럼 타입과 collation 불일치는 EXPLAIN의 `ref` 컬럼 값이나 `Using join buffer`로 잡힌다.
...
이 흐름에서 "느리면 무조건 인덱스를 추가한다"는 식의 접근은 피한다. 현재 인덱스를 왜 안 탔는지부터 설명하고, leftmost prefix 위반인지, 함수/형 변환인지, cardinality가 낮은 prefix인지 진단하는 것이 올바른 순서다.
...
이 운영 흐름까지 묶어 설명하면 인덱스 이론을 실제로 적용해본 설계임이 분명하게 드러난다.

## 인덱스 진단 4스텝

EXPLAIN/인덱스 문제는 다음 4스텝으로 푼다.

1. 문제 정의 — 어떤 화면/API가 느렸고, 어떤 지표(P95/P99, slow query log)에서 잡았는지.
2. 진단 — `EXPLAIN`을 보고 `type`, `key`, `Extra` 중 어떤 신호가 문제였는지. cardinality, leftmost prefix, 함수 적용, OFFSET 깊이 같은 구체 원인을 짚는다.
3. 처리 — 인덱스 추가/변경, 쿼리 리라이팅, keyset pagination, covering index 중 무엇을 적용하고 왜 그것을 골랐는지.
4. trade-off — 추가한 인덱스의 쓰기 비용, 통계 영향, 인덱스 개수 관리. "안 만든 인덱스"에 대한 판단도 같이 본다.

이 4스텝은 거의 모든 DB 성능 문제의 답을 만들어낸다. 추상적인 "인덱스를 잘 써야 한다" 톤을 피하고, 위 시나리오 중 하나를 구체적인 사례로 끌어올 수 있게 준비한다.

실제 사례로 연결할 때는 "주문 목록 OFFSET pagination을 cursor 방식으로 바꿔 P99를 N% 개선했다", "쿠폰 조회 인덱스 prefix를 status에서 member\_id로 바꿔 평균 응답 시간을 줄였다"처럼 정량 표현 한 줄을 꼭 포함시킨다. 숫자가 없으면 단순 학습으로 받아들여진다.
...
- "왜 인덱스를 안 만들었는가"도 설명할 수 있어야 한다.

## 2026-05-19 보강 — 주문·쿠폰·메뉴 조회 인덱스 패턴

F&B 커머스 조회 성능 설계는 구체적인 화면을 잡고 시작하면 설득력이 좋아진다. 예를 들어 “내 주문 목록”, “사용 가능한 쿠폰”, “매장별 판매 메뉴”는 모두 where 조건과 정렬 기준이 고정된 API이므로 복합 인덱스 설계 문제로 바꿔 말할 수 있다.
...
EXPLAIN을 볼 때는 `key`가 예상 인덱스인지, `rows`가 화면 규모에 비해 과도하지 않은지, `Extra`에 `Using filesort` 또는 `Using temporary`가 반복되는지부터 본다. 마무리는 “인덱스 추가 전후 slow query p95/p99와 실행 계획을 비교하고, 쓰기 비용과 배치 영향까지 같이 확인한다”로 정리한다.
