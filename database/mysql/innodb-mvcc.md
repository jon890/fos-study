...
### Purge Thread와 History List Length
...
### 일관 읽기(Consistent Read = Snapshot Read)
...
### 현재 읽기(Current Read = Locking Read)
...
## MVCC + Redo Log + Crash Recovery

### Undo Log와 Redo Log의 역할 분담
...
```mermaid
flowchart TD
    A["1 · 트랜잭션 시작: 새 DB_TRX_ID 할당"] --> B["2 · 변경 직전: Undo Log에 이전 버전 기록"]
    B --> B1["Undo Log 자체도 Redo Log에 기록<br/>(크래시 복구 시 Undo Log 복원이 필요)"]
    B1 --> C["3 · 버퍼 풀 페이지 수정 (메모리)"]
    C --> C1["DB_TRX_ID, DB_ROLL_PTR 업데이트"]
    C1 --> D["4 · Redo Log Buffer에 변경사항 기록 (메모리)"]
    D --> E["5 · 커밋 요청: Redo Log Buffer를 디스크에 fsync<br/>(innodb_flush_log_at_trx_commit=1)"]
    E --> E1["fsync 완료 시 커밋 완료 응답"]
    E1 --> F["6 · 변경 페이지는 나중에 Checkpoint 때 디스크 반영"]
```
...
```mermaid
flowchart TD
    R[재시작] --> S["Redo Log 스캔 (Roll-Forward)"]
    S --> S1["커밋 마커 있음 → Redo 적용<br/>(디스크에 누락된 변경사항 복원)"]
    S --> S2["커밋 마커 없음 → 미완료 트랜잭션으로 표시"]
    S1 --> U["Undo Log로 미완료 트랜잭션 롤백 (Roll-Back)"]
    S2 --> U
    U --> U1["커밋 전 크래시된 트랜잭션의 변경을 이전 버전으로 복원"]
    U1 --> E[서비스 재개]
```
