...
# OpenSearch 기초: 검색 엔진을 백엔드 관점에서 다루기

## 왜 지금 이 주제인가

관계형 DB만으로 운영하다가 검색 기능이 본격적으로 필요해지는 순간이 꼭 온다. 다음 같은 요구가 쌓이면 `LIKE '%...%'` + 인덱스는 금세 깨진다.
...
`FULLTEXT` 인덱스를 동원해도 한국어 형태소와 다국어 처리, 실시간 색인 토폴로지, 분산 집계까지 가면 MySQL 단독으로는 무겁다. 대규모 커머스 플랫폼처럼 상품 카탈로그와 검색/추천/로그 분석이 동시에 돌아가는 도메인이라면 OpenSearch(또는 Elasticsearch) 계열은 사실상 표준 선택지다.
...
대규모 이커머스 도메인에서 OpenSearch를 얹는 방식은 보통 이렇게 자리 잡는다.
