...
예를 들어 검색 쿼리 `"스타벅스 매장 영업시간"`이 들어왔을 때, standard analyzer는 공백으로만 자르기 때문에 `[스타벅스, 매장, 영업시간]` 같은 토큰이 만들어진다. 문제는 사용자가 `"스타벅스매장"`처럼 붙여 쓰면 하나의 토큰 `스타벅스매장`이 되어버려 색인된 `스타벅스`과 매칭되지 않는다. 한국어 검색에서 nori나 ngram이 필요한 이유가 여기에 있다.
...
`stoptags`는 제거할 품사 태그다. `J`(조사), `E`(어미), `MAG`(일반 부사) 같은 것들은 검색 매칭에 도움이 안 되기 때문에 제거한다. 이 리스트는 Lucene 문서에 전부 정의되어 있으니 프로젝트마다 조정하면 된다.
...
- `"스타벅스"`을 `min_gram: 2, max_gram: 3`으로 넣으면 `[스타, 스타벅, 타벅, 타벅스, 벅스]`이 된다.

이 덕분에 `"타벅스"`으로 검색해도 `"스타벅스"`이 매칭된다. 즉, 부분 일치(wildcard 없는 `LIKE '%xxx%'`와 유사한 UX)를 구현할 수 있다.
...
**자동완성 기능에는 거의 항상 `edge_ngram`이 정답이다.** 사용자가 `"스"`를 입력하면 `"스타벅스"`, `"스벅"`이 떠야 하지만, `"타벅"`만 입력했는데 `"스타벅스"`이 떠서는 자동완성 UX가 어색해진다.

### 색인 시점 vs 검색 시점 analyzer

ngram을 설계할 때 가장 많이 하는 실수가 **색인 analyzer와 검색 analyzer를 동일하게 쓰는 것**이다.

- 색인 시점: 문서의 `"스타벅스"`을 `[스, 스타, 스타벅, 스타벅스]`로 쪼개 저장
- 검색 시점 사용자가 `"스타"`라고 입력했을 때 또 edge_ngram을 적용하면 `[스, 스타]`로 쪼개져 두 토큰 모두 매칭을 시도한다.

만약 검색 시점에도 edge_ngram을 쓰면, `"스타"`가 `[스, 스타]`로 확장되고, 이 중 `"스"`은 전혀 다른 상품(`"스벅세일"`의 `스` 부분)과도 매칭되어 정확도가 급락한다. 그래서 일반적으로 다음 패턴을 쓴다.
...
1. `decompound_mode`가 기본값(`discard`)이라 원본 단어가 사라진다. `"스타벅스"`이 사전에 없거나 다르게 분해되면 `"스타벅스"`이라는 완전한 토큰이 인덱스에 아예 없다.
...
결과로 `[스, 스타, 스타벅, 스타벅스]`이 나오면 색인이 제대로 되는지 쉽게 눈으로 확인할 수 있다.
