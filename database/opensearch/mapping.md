...
- **Complex Types**:
  - `object`: JSON 객체 (기본값)
  - `nested`: 객체 배열을 독립적으로 쿼리해야 할 때 사용 (내부적으로 별도 문서로 색인)
- **Geo Types**:
...
이 두 타입의 차이는 **데이터를 저장할 때 분석기(Analyzer)를 거치느냐, 거치지 않느냐**에 있다. <br>
...
- **용도**:
  - 본문 검색, 이메일 내용, 제품 설명 등 긴 텍스트의 내용을 검색할 때
- **특징**:
...
- **동작 원리**:
  ...
  - 분석기를 거치지 않는다
- **용도**:
  - ID, 이메일 주소, 상태 코드, 태그, 카테고리 등 정확한 일치가 필요한 필드
- **특징**:
...
### 타입이 어긋나면 에러가 아니라 조용히 안 맞는다

직접 겪은 쪽은 조금 달랐다. 검색이 아니라 **삭제**에서 드러났다.

문서 ID 를 `keyword` 로 매핑해 두고, 애플리케이션에서는 그 값을 `Long` 으로 들고 있었다.
"이 목록에 없는 문서는 지운다" 는 정리 작업을 `mustNot` 과 `Terms` 쿼리로 짰는데,
쿼리에 `Long` 을 그대로 실어 보냈다.

```java
// 문제 — id 가 Long 인 채로 나간다
ids.stream().map(FieldValue::of)

// 수정 — keyword 는 문자열이므로 맞춰서 보낸다
ids.stream().map(id -> FieldValue.of(String.valueOf(id)))
```

**아무 에러도 나지 않았다.**
`keyword` 는 들어온 값을 그대로 단일 토큰으로 저장하므로,
색인된 `"123"` 과 쿼리의 `123` 은 그냥 서로 다른 값일 뿐이다.
매칭이 안 될 뿐 실패가 아니라서 로그에도 남지 않는다.

`mustNot` 안에서 이게 터지면 조건이 통째로 뒤집힌다.
"이 목록에 있는 것은 빼고 지운다" 가 "아무것도 안 걸리니 전부 지운다" 가 된다.

`text` 와 `keyword` 를 고르는 이야기는 보통 "부분 검색이 되냐 안 되냐" 로 끝나는데,
실제로 비싼 값을 치른 건 **쓰기·삭제 경로에서 타입이 어긋난 것을 아무도 알려주지 않는다**는 쪽이었다.
정렬·집계 때문에 `keyword` 를 골랐다면, 애플리케이션에서 그 필드를 다루는 타입도 함께 고정해 두는 편이 안전하다.
...
실무에서는 하나의 필드를 검색 용도와 정렬/집계 용도로 모두 사용해야 하는 경우가 많다 <br>
이 때 `fields` 속성을 사용한다
...
- **이유**: OpenSearch(Lucene)의 역색인(Inverted Index) 구조는 불변이다.
  - 이미 `text`로 토큰화되어 저장된 데이터를 나중에 `keyword`나 `long`으로 바꾸려면, 저장된 모든 데이터를 다시 읽어서 새로운 방식으로 저장 구조를 다시 짜야 한다.
- **가능한 경우**:
  - 새로운 필드를 **추가**하는 것은 가능하다
- **해결책: Reindexing**(재색인)
