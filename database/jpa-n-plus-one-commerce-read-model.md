# JPA N+1과 커머스 조회 모델: 주문/메뉴/쿠폰 도메인에서 살아남기
...
외식 프랜차이즈 도메인은 N+1이 더 잘 터지는 구조다. 매장(브랜드/지점) × 메뉴(베이스 상품) × 옵션(사이즈, 토핑, 사이드) × 쿠폰(적용 가능 여부) 조합이 항상 묶여서 다닌다. "주문 상세 조회 한 번에 200쿼리"가 농담이 아니라 실측치로 잡힌다.

이 문서는 N+1을 단순히 "fetch join 쓰면 된다"로 끝내지 않고, **언제 fetch join을 쓰면 안 되는지**, **언제 read model을 분리해야 하는지**, **언제 JPA를 버리고 MyBatis로 가는지** 까지 정리한다. "JPA N+1을 어떻게 해결하는가"라는 물음은 결국 ORM의 한계를 어디까지 이해하고 있느냐를 가르는 지점이다.
...
CQRS의 가벼운 버전이다. 굳이 명령/조회를 별도 DB로 분리하지 않아도, **레이어를 나누는 것만으로** 충분히 효과가 크다.
...
정리하면 이렇다. OSIV는 개발 편의를 위한 기본값이지 운영 권장 값이 아니다. OSIV를 false로 두고, 서비스 계층에서 필요한 LAZY를 명시적으로 초기화하거나 DTO projection으로 처리하는 것이 안전하다.

## MyBatis vs JPA 선택 기준

JPA를 쓴다고 모든 쿼리를 JPA로 풀어야 한다는 법은 없다. 외식 도메인 관점에서 정리하면 다음과 같다.
...
외식 프랜차이즈처럼 매장(브랜드/지점) × 메뉴 × 옵션 × 쿠폰 그래프가 깊은 환경에서는 다음 분리가 거의 정석이다.
...
읽기 비중이 큰 정적 마스터 데이터 캐시에서 조회와 갱신을 분리하는 설계는 본 결정 트리의 "캐시 + 조회 전용 Reader" 라인에 그대로 매핑된다. 일반화하면 이렇다.

> 정적 설정 데이터는 StampedLock + RabbitMQ Fanout으로 다중 서버 캐시 정합성을 잡고, 캐시 충족 판정은 별도 Reader 쿼리로 분리해서 운영한다. 동일한 구조를 F&B 메뉴/매장/프로모션 캐시에도 적용할 수 있다. 쓰기 도메인은 JPA로, 조회 모델은 캐시 + 전용 SQL Reader로 분리한다.
...
N+1은 단순히 쿼리가 많다는 문제가 아니다. 운영 지표를 단계적으로 망가뜨린다. F&B/e-Commerce 도메인은 이벤트 오픈, 점심·저녁 피크, 쿠폰 발급 같은 트래픽 스파이크가 명확해서 N+1의 영향이 더 잘 드러난다.
...
graceful shutdown 503 대응에서 쓰는 "운영 제약을 시간 예산으로 환산하는 사고"가 여기서도 그대로 적용된다. 일반화하면 이렇다.

> graceful shutdown 사례에서 30s 예산 안에 preStop 15s + gRPC grace 12s + 여유 3s로 분배하듯, N+1 운영 대응도 'Connection Pool 고갈까지 몇 초 남았는가'를 먼저 계산하고 임시 차단을 결정한다. 코드 수정 없이 N+1을 해결하려 하지 않는다.
...
### Q1. JPA에서 N+1을 어떻게 해결하는가

N+1은 단일 해결책이 아니라 케이스별 도구 선택 문제다.
- 단건 상세는 fetch join이나 EntityGraph로 한 번에 끌어온다. 단 컬렉션 둘 이상이면 한쪽은 batch fetching에 맡긴다.
- 목록 + 컬렉션은 fetch join을 쓰면 안 된다. 페이징이 메모리에서 일어나기 때문이다. 이 경우 `default_batch_fetch_size`를 설정해 IN 쿼리로 묶거나, DTO projection으로 응답 모델을 직접 만들고 부족한 컬렉션은 별도 IN 쿼리로 한 번 더 가져오는 2단계 전략을 쓴다.
- 화면이 복잡한 카드 리스트나 통계는 아예 조회 전용 Reader를 두고 MyBatis로 처리한다. 쓰기 모델과 조회 모델을 분리하는 것이 핵심이다.

### Q2. fetch join과 EntityGraph 차이

동작 원리는 같다. 둘 다 한 SQL로 join 해서 가져온다. 차이는 선언 위치다. fetch join은 JPQL에 명시적으로 들어가고, EntityGraph는 메서드 시그니처 위 어노테이션으로 선언한다. 같은 Repository 메서드에 다양한 fetch 전략을 두고 싶을 때는 EntityGraph가 깔끔하고, 동적 join이 필요하면 QueryDSL fetch join을 쓴다.

### Q3. OSIV는 어떻게 두는가

운영 환경에서는 false가 기본이다. true는 LAZY 접근을 어디서든 허용해서 편하지만, 트랜잭션 밖에서 커넥션을 잡고, 외부 API 호출 중에도 커넥션이 묶여서 풀이 빠르게 고갈된다. false로 두면 서비스 계층에서 필요한 데이터를 명시적으로 가져와야 하기 때문에, 자연스럽게 N+1 예방이 된다.

### Q4. JPA만 쓰지 않고 MyBatis도 함께 쓰는 이유

도메인 규칙이 들어가는 쓰기는 JPA가 강하다. 상태 전이, 무결성 제약, 도메인 이벤트가 깔끔하다. 반면 보고서, 통계, 동적 검색, 카드 리스트처럼 SQL 가독성과 join 자유도가 중요한 조회는 MyBatis가 단순하다. 한 도구로 모든 걸 풀려고 fetch join을 누더기로 깁기보다, 조회 일부를 MyBatis로 분리해 쓰기 모델의 일관성을 지키는 쪽이 운영에 더 좋다.

### Q5. 주문 목록을 페이징하면서 라인까지 보여줘야 하는데 fetch join을 쓰면 안 된다면

컬렉션 fetch join + 페이징은 메모리 페이징이 일어나서 위험하다. 두 가지 중 하나를 쓴다. 첫째, `default_batch_fetch_size`를 설정해 페이지 안에서 라인을 IN 쿼리로 묶어서 가져오게 한다. 둘째, 헤더 DTO를 먼저 페이징으로 가져오고 헤더 ID 목록을 모아서 라인을 별도 IN 쿼리로 한 번 더 가져온 뒤, 메모리에서 group by로 합친다. 응답 모델이 복잡하면 후자가 낫다. 페이징 정확성과 인덱스 활용 모두 명시적으로 보장된다.

### Q6. F&B 도메인에서 주문 카드 목록 설계

모바일 앱 주문 내역 화면을 예로 든다. 회원이 최근 주문 20건을 카드로 본다. 카드 한 장에는 주문 헤더 + 라인 미리보기 + 쿠폰명이 들어간다. 두 단계 조회로 푼다. 1단계는 JPQL DTO projection으로 헤더 + 첫 라인 메뉴명 + 적용 쿠폰명을 평탄한 행으로 페이징해서 가져온다. 2단계는 헤더의 orderId 목록으로 라인을 IN 쿼리 한 번에 가져와 group by로 합친다. 컬렉션 fetch join + 페이징은 메모리 페이징이라 절대 쓰지 않는다. 쿼리는 2회로 고정되고, 회원의 주문이 1만 건이어도 한 페이지만큼만 메모리에 올라온다.

### Q7. 주문 상세 조회에서 매장·메뉴·옵션·쿠폰을 모두 보여줄 때 쿼리 몇 개로 끝나는가

단건 상세는 컬렉션 fetch join이 안전하므로 `@EntityGraph(attributePaths = {"store", "member", "lines", "lines.menu"})`로 한 SQL에 매장·회원·라인·메뉴까지 끌어온다. 옵션과 쿠폰은 라인 컬렉션과 같은 깊이의 별도 컬렉션이라 같이 fetch join하면 카르테시안이 폭발한다. 이 두 개는 `default_batch_fetch_size`에 맡겨 IN 쿼리 1\~2회로 정리한다. 총 3\~4쿼리로 끝난다. 단건 조회는 이 정도면 충분하고, 그 이상 줄이려고 SQL을 수공으로 짜기 시작하면 유지보수가 깨진다.

### Q8. 이벤트 오픈 직후 메뉴 카탈로그 조회가 느려졌을 때 원인 좁히기

먼저 APM에서 가장 느린 trace 한 건을 펼쳐 SQL span 패턴을 본다. 같은 패턴의 select가 N번 반복되면 N+1이다. 그다음 Hikari `active`/`pending`을 확인해 Connection Pool wait가 늘었는지 본다. wait가 늘었으면 응답당 평균 쿼리 카운트의 평소 대비 추이를 본다. 평소 5쿼리였는데 50쿼리로 점프했으면 직전 배포로 fetch 전략이 무너졌을 가능성이 높다. 임시 mitigation은 코드 수정 없이는 어려우니, 문제 화면 traffic을 feature flag로 잠시 차단하거나 캐시 TTL을 일시 늘려 DB 부하를 줄인다. 영구 해결은 N+1을 수정하는 PR이다.

### Q9. F&B 도메인에서 MyBatis로 분리하는 판단 기준

읽기 비중이 큰 정적 마스터 데이터의 캐시 충족 판정 쿼리를 복합 인덱스 튜닝하면서 raw SQL을 직접 다루는 경우가 그 출발점이다. F&B 도메인에서 MyBatis 도입 기준은 단순하다. 1) 컬렉션이 둘 이상 같이 페이징되어야 한다, 2) 동적 검색 조건이 많다, 3) group by + window function이 필요하다, 이 세 중 둘이 만나면 JPA로 누더기를 깁기 전에 MyBatis Reader로 분리한다. 모든 SQL을 MyBatis로 옮기지는 않는다. 쓰기는 그대로 JPA에 두고, 조회 한두 화면만 MyBatis로 빼는 게 운영에 가장 정직한 분업이다.

### Q10. 주문 목록 응답에 평균 30쿼리가 찍힌다면 어디부터 손대는가

가장 먼저 trace 하나를 펼쳐 N+1 패턴인지 카르테시안 곱인지 구분한다. 같은 SQL이 반복되면 N+1, 행 수가 부풀어 distinct 메모리 처리가 보이면 카르테시안이다. N+1이면 우선순위는 1) 컬렉션 fetch join + 페이징 제거, 2) `default_batch_fetch_size` 적용, 3) DTO projection으로 응답 모델 직조, 4) 조회 전용 Reader 분리 순으로 진행한다. 카르테시안이면 컬렉션 둘을 동시에 join하지 않게 한쪽을 batch fetching에 맡기거나 2단계 IN 쿼리로 푼다. 모든 변경에는 `SQLStatementCountValidator`로 회귀 가드를 같이 단다. 운영에서 N+1을 잡는 핵심은 다시 들어오지 않게 하는 것이다.

### 30초 요약

N+1은 LAZY × 컬렉션 × 순회의 함수다. 단건 상세는 fetch join, 목록은 DTO projection + 2단계 IN 쿼리, 컬렉션 둘 이상은 batch fetching, 보고서는 별도 Reader로 분리한다. OSIV는 false가 운영 기본값이다. 가장 중요한 건 회귀 방지로, 통합 테스트에 쿼리 카운트 assertion을 박아 CI에서 깨지게 한다.

### 1분 요약

읽기 비중이 큰 정적 마스터 데이터 캐시를 다중 서버 정합성으로 분리하는 구조는 F&B 메뉴/매장/프로모션 캐시에도 그대로 적용된다. 쓰기 도메인은 JPA로, 조회 모델은 캐시 + 전용 Reader로 분리한다. N+1은 단일 해결책이 아니라 케이스별 도구 선택 문제다. 단건 상세는 EntityGraph, 목록은 DTO projection + 2단계 IN 쿼리, 카드 리스트가 복잡해지면 MyBatis Reader로 분리한다. 페이징과 컬렉션 fetch join은 절대 같이 쓰지 않는다. 운영 관측성은 요청당 쿼리 카운트를 SLI로 두고 임계값을 넘으면 traceId와 함께 경고를 띄운다. 회귀 방지는 통합 테스트에 `SQLStatementCountValidator`로 박아 CI 단에서 깨지게 한다.

### 정적 마스터 데이터 캐시 경험 → F&B 도메인 매핑

| 정적 마스터 데이터 캐시 경험 | F&B/e-Commerce 도메인 적용 |
|------------------|------------------------------|
| 정적 데이터 캐시 + RabbitMQ Fanout 다중 서버 정합성 | 메뉴/매장/프로모션 캐시 정합성 |
| StampedLock + `tryReadLock(2.5s)` | 메뉴 갱신 중 조회 차단 최소화 |
| 캐시 충족 판정 별도 SQL Reader | 매장 운영 콘솔 주문 현황 Reader |
...
| 추상 테스트 클래스 기반 대규모 테스트 스위트 | 쿼리 카운트 assertion 통합 테스트 |
