...
요구사항: 주문 1건 + 라인 N개 + 라인별 옵션 M개 + 매장 정보 + 적용 쿠폰을 한 화면에 보여준다.
...
이 코드는 주문 1쿼리 + store 1 + member 1 + lines 1 + 라인별 menu N + 라인별 options N + coupons 1 식으로 쿼리가 폭발한다. 라인이 5개면 13쿼리, 20개면 43쿼리.
