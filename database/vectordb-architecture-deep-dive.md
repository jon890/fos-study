...
[벡터 DB 어떻게 고를까](./vectordb-comparison.md)에서 기능·운영 관점으로 5종을 비교했다면, 이 글은 그 다음 단계다 — **표면 기능 밑에서 실제로 무엇이 다른지**를 뜯어본다.
...
pgvector 는 그 좁은 선택지 안에서도 빌드 메모리 제약이 하나 더 붙는다. HNSW 인덱스를 빌드하려면 그래프 구조를 메모리에 들고 작업해야 하는데, 이때 쓰는 메모리 예산이 `maintenance_work_mem` 이다. 그래프가 이 예산을 넘으면 PostgreSQL 이 일부를 디스크 임시 파일로 밀어내는데(스필), `work_mem` 초과 시 정렬이 external merge sort 로 디스크를 쓰는 것과 같은 PostgreSQL 의 일반 패턴이다. 디스크 접근이 메모리보다 몇 자릿수 느려서, 스필이 시작되면 빌드 시간이 수 배에서 수십 배까지 늘어난다.
...
- **pgvector**: 여기서 가장 분명한 한계가 나온다.
  **단일 노드 쓰기만 지원한다.**
  읽기 복제는 되지만 쓰기는 단일 프라이머리에 묶인다.
  Citus 로 샤딩해도 원인은 같은 데서 나온다 — Citus 는 벡터 유사도가 아니라 임의의 해시 키로 row 를 샤드에 흩뿌리므로, 비슷한 벡터가 여러 샤드에 무작위로 흩어진다. 각 샤드가 독립된 HNSW/IVFFlat 인덱스를 따로 가져, 진짜 최근접 이웃을 찾으려면 모든 샤드를 다 뒤져 합쳐야 한다(OpenSearch 의 shard fan-out 과 같은 구조). 일부 샤드만 보면 recall 이 떨어지고 전부 보면 샤딩의 이점이 사라지니, 결국 recall 이 떨어지거나 오히려 느려지는 두 갈래뿐이다.
