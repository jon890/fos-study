...
## 핵심 질문 5: RCC 시스템 — RTP 편차 보정을 위한 비동기 생성과 타입별 추상화
...
RCC의 본질은 응답성 최적화가 아니라 **"유저 경험 품질을 유지하기 위한 결과 보정"**이라는 점을 먼저 짚는 게 중요합니다. 이 관점에서 커머스에 비슷한 패턴이 적용될 수 있는 케이스는 추천 결과가 너무 나쁠 때 큐레이션된 대체 결과로 폴백하는 **추천 품질 보정**, 재고 소진 상품을 조회한 유저에게 미리 준비된 대체 상품을 제안하는 **대체 상품 폴백**, 개인화 데이터가 부족한 신규 유저에게 세그먼트 기반 결과를 제공하는 **개인화 콜드 스타트 보정** 같은 시나리오입니다. 구조적으로는 백그라운드 생성(@Async), 타입별 생성 조건 추상화(Strategy Pattern), 생성 여부 모니터링이 공통적으로 적용됩니다. 핵심은 성능이 아니라 **"확률적으로 일부 유저가 받는 나쁜 경험을 구조적으로 걸러내는 장치"**라는 목적 자체를 정확히 전달하는 것입니다.
