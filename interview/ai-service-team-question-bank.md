...
- AI 서비스 개발팀에서 직접 설계·운영한 경험을 시니어 Java 백엔드 면접 관점에서 구술 가능한 형태로 정리한다 — (1) Spring Batch 11 Step 기반 Confluence → OpenSearch RAG 색인 파이프라인, (2) gRPC OCR 서버 배포·스케일인 503의 graceful shutdown 예산 재설계, (3) 다중 서버 인메모리 캐시 정합성(RabbitMQ Fanout + StampedLock), (4) 12일 단독 풀스택 AI 웹툰 MVP의 4인 에이전트 팀 하네스·Gemini 모델 전략·환각 차단.
- 각 경험은 단순 사용 경험이 아니라 트레이드오프 기반의 설계 의사결정으로 풀어 답할 수 있도록 구성한다 — 왜 JobExecutionContext가 아니라 @JobScope, 왜 30초 안에서 'sleep 15 + grace 12'였는지, 왜 Redis가 아니라 인메모리였는지, 왜 SSE가 아니라 Promise.allSettled로 바꿨는지를 1분 답변과 압박 방어 답변에 모두 녹인다.
- 추가로 1분 자기소개와 지원 동기(왜 변화 / 왜 이 회사 / 왜 이 직무)를 별도 섹션으로 두어, 본 질문 뱅크가 'AI/에이전트 협업 경험을 단순 툴 사용자가 아니라 파이프라인·아키텍처 설계자 수준으로 드러내는 시니어 면접 자료'로 사용 가능하도록 한다.

## 1분 자기소개 준비

- NHN에서 4년째 Spring Boot 기반 MSA 환경에서 백엔드를 개발해온 김병태입니다. 처음 3년은 스포츠 베팅과 소셜 카지노 슬롯 게임 백엔드에서 신규 게임 개발과 성능 개선·아키텍처 재설계를 담당했고, 이후 AI 서비스 팀으로 이동해 사내 RAG를 위한 Spring Batch 색인 파이프라인을 설계·운영했습니다.
- 기술적으로는 다중 서버 인메모리 캐시 정합성을 RabbitMQ Fanout + Hibernate PostCommitUpdateEventListener + StampedLock으로 직접 해결했고, 팀에서 운영하던 Kafka 이벤트 드리븐(커밋 후 이벤트 발행 기반 비동기 처리) 구조도 함께 다뤄봤습니다.
- AI 서비스 팀에서는 Confluence → OpenSearch RAG 파이프라인을 11개 Step으로 분리하고 [AsyncItemProcessor](../java/spring-batch/async-item-processor.md)로 I/O를 병렬화하는 등 운영 가능한 형태로 정리했습니다. 직전에는 12일 동안 혼자 AI 웹툰 제작 도구 MVP를 풀스택으로 구현하며 199 plan / 760 커밋을 4인 에이전트 팀 하네스로 처리했고, 이 과정에서 'AI 도구를 쓰는 사람'이 아니라 'AI 호출 구조와 비용 모델을 설계하는 사람'으로 한 단계 더 나갔다고 생각합니다.
- 이 모든 경험을 1,600만 고객이 사용하는 커머스 트래픽 환경에서 검증하고 싶어 지원하게 되었고, 안정성과 개발 속도를 동시에 끌어올리는 데 빠르게 기여하고 싶습니다.

## 지원 포지션 맞춤 연결 포인트

- 다중 서버 인메모리 캐시 정합성을 RabbitMQ Fanout + Hibernate PostCommitUpdateEventListener + StampedLock으로 직접 해결한 경험은, 기술 블로그의 '변경 빈도/라이프사이클 기반 [Cache-Aside](../database/redis/cache-aside.md) + Kafka 하이브리드' 의사결정과 같은 결이라 합류 직후 같은 패턴을 1,600만 트래픽 규모에서 검증할 수 있다.
- 팀에서 운영하던 Kafka 이벤트 드리븐(커밋 후 이벤트 발행 기반 비동기 처리) 구조를 접한 경험은, '무중단 OAuth2 전환' 사례에서 본 Resilience4j 3단계 보호 + Feature Flag + Shadow Mode 같은 운영 안정성 설계와 같은 결의 사고방식이다.
...
### 왜 이 회사인가
- 기술 블로그의 'MSA 데이터 연동 전략' 글에서 데이터의 변경 빈도·라이프사이클을 기준으로 Cache-Aside와 Kafka 이벤트 드리븐을 하이브리드로 묶은 부분이, 내가 스포츠 베팅 백엔드에서 RabbitMQ Fanout + StampedLock으로 풀었던 다중 서버 캐시 정합성 문제와 같은 결의 의사결정이라 강하게 공감됐고, 같은 의사결정 패턴을 1,600만 트래픽 규모에서 검증하고 싶다.
- '무중단 OAuth2 전환' 글에서 Feature Flag · Shadow Mode · Resilience4j 3단계 보호 · Jitter 분산까지 운영 안정성을 '코드'와 '배포 전략'을 함께 설계해서 푸는 방식이, 팀에서 운영하던 Kafka 커밋 후 이벤트 발행 기반 비동기 처리 구조를 접했던 경험과 결이 같아 빠르게 합류 가능하다고 판단했다.
...
- I/O 바운드인 임베딩/문서 파싱 호출은 AsyncItemProcessor + AsyncItemWriter로 청크 내 병렬화했고, Reader(ConfluencePageItemReader)는 ItemReader + StepExecutionListener를 구현해 beforeStep에서 스페이스별 연결 정보로 API 서비스를 구성하고 커서 기반 페이지네이션으로 목록을 읽도록 설계했다.
...
- I/O 바운드인 임베딩 호출은 AsyncItemProcessor + AsyncItemWriter로 청크 내 병렬화했고, Reader는 ItemReader + StepExecutionListener를 구현해 beforeStep에서 스페이스별 연결 정보를 읽어 커서 기반 페이지네이션으로 처리하도록 했다.
...
- '재시작 시 503은 어쩔 수 없는 거 아니냐'로 들어오면, Envoy drain과 upstream graceful shutdown을 시간 축에서 정렬되게 재설계하면 종료 구간의 ECONNREFUSED 창이 닫힌다는 점을 종료 시퀀스 표(T+15s SIGTERM → grace 12s)로 설명한다.
...
## 메인 질문 3. 다중 서버 환경에서 인메모리 캐시 정합성을 RabbitMQ Fanout + Hibernate 이벤트 + StampedLock으로 푼 설계를 설명해주세요. 왜 Redis가 아니라 인메모리였고, 갱신 중 동시성은 어떻게 보장했나요?
...
- **[스포츠 베팅팀, 2023] 첫 구축**: 정적 설정 데이터를 인메모리 Map/Ehcache에 캐싱하면서 어드민 변경을 다중 서버에 반영해야 했다. 어드민이 데이터를 바꾸면 reloadMemory가 MQ Fanout(FANOUT_STATIC_DATA)으로 변경된 데이터 타입 식별자를 broadcast하고, 각 서버가 자기 큐에서 받아 해당 타입 캐시만 선택 갱신하는 구조로 정합성을 유지했다. 갱신 중 조회 보호는 `ReentrantReadWriteLock`(reload=writeLock / 조회=readLock)으로 했다. (MQ 벤더 이중화 자체는 인프라 담당 몫이었고, 내가 맡은 건 그 위의 캐시 리로드 공통 기반과 읽기 보호였다.)
- **[슬롯팀, 2024+] 같은 패턴을 발전**: 트리거를 어드민 API에서 Hibernate `PostCommitUpdateEventListener`로 바꿔 트랜잭션 커밋 이후에만 RabbitMQ Fanout으로 변경 ID를 발행했고, 읽기 보호는 `ReentrantReadWriteLock`을 `StampedLock`의 tryReadLock(2.5초 타임아웃)으로 교체해 갱신 중 조회의 대기 상한을 명시화했다.
- **[슬롯팀] 추상화**: 새 캐시 타입을 추가할 때마다 같은 동시성 문제가 반복되지 않도록 `StaticDataManager` 인터페이스(init/refresh/clear)로 책임을 분리하고 데이터 타입별 구현체로 확장해, 신규 타입 추가 시 기존 코드를 건드리지 않게 했다.
- 왜 Redis가 아니라 인메모리였는가의 트레이드오프도 명확하다 — 정적 설정 데이터는 변경 빈도가 매우 낮고 핫 패스에서 매 요청 조회되기 때문에, 네트워크 왕복을 없애는 인메모리 + 변경 시 invalidate가 비용 대비 효과가 가장 컸다. 단점인 다중 서버 정합성은 Fanout 메시징으로 해결했다.
- 이 경험은 기술 블로그의 'MSA 데이터 연동 전략'에서 변경 빈도/라이프사이클 기준으로 Cache-Aside와 Kafka 이벤트 드리븐을 하이브리드로 묶은 의사결정과 같은 결이다 — 같은 의사결정 패턴을 1,600만 트래픽 규모에서 적용·검증할 수 있다.

### 1분 답변 구조

- 다중 서버 인메모리 캐시 정합성을 두 팀에 걸쳐 풀었다 — 스포츠 베팅팀(2023)에서 어드민 변경을 MQ Fanout(데이터 타입 식별자)으로 broadcast하고 `ReentrantReadWriteLock`으로 갱신 중 읽기를 보호하는 구조를 처음 구축했다.
- 슬롯팀(2024+)에서 같은 패턴을 발전시켰다 — 트리거를 Hibernate `PostCommitUpdateEventListener`(커밋 후 발행)로 바꾸고, 읽기 보호를 `StampedLock` tryReadLock(2.5초 타임아웃)으로 교체해 조회 대기 상한을 명시화했으며, `StaticDataManager` 인터페이스로 타입별 확장 구조를 잡았다.
- Redis가 아닌 인메모리를 택한 건 변경 빈도와 핫 패스 특성 때문이었고, 이는 기술 블로그의 '데이터 라이프사이클 기반 하이브리드 연동' 의사결정과 같은 결로, 같은 패턴을 커머스 트래픽 규모에서 검증하고 싶다.
...
**F3-1.** StampedLock 대신 ReentrantReadWriteLock이나 Versioned Atomic Reference도 후보였을 텐데, 왜 StampedLock이었고 tryReadLock 타임아웃을 2.5초로 잡은 근거는 무엇이었나요?

**F3-2.** Fanout Exchange로 broadcast하면 N대 인스턴스가 동시에 같은 데이터를 갱신 요청하게 되는데, 외부 소스(DB/원본)에 발생하는 부하는 어떻게 평가하고 제어했나요?

**F3-3.** PostCommitUpdateEventListener는 트랜잭션 커밋 이후라도 메시지 발행이 실패하면 데이터-메시지 정합성이 깨질 수 있는데, 이 경계는 어떻게 다뤘나요?

**F3-4.** 정적이지만 '거의 안 변하는' 데이터와 '꽤 자주 변하는' 데이터의 경계를 어디에 두었나요? 그 기준이 잘못 잡혔던 사례가 있었나요?

**F3-5.** 이 구조를 커머스 도메인(예: 전시 룰, 상품 진열 정책)에 옮긴다면 어디를 그대로 가져가고 어디를 바꿔야 한다고 보시나요?

---

## 메인 질문 4. 12일 단독 풀스택으로 AI 웹툰 MVP를 만들면서 4인 에이전트 팀(planner/critic/executor/docs-verifier)을 어떻게 조율했고, Gemini 모델 fallback과 환각 차단을 어떻게 설계했는지 설명해주세요. 이 경험이 커머스 백엔드 업무에 어떻게 옮겨질 수 있다고 보시나요?
...
**F4-1.** 4인 에이전트 팀에서 'critic'을 별도 역할로 둔 이유는 무엇이었고, 같은 모델임에도 시야 차이가 실제로 발견된 사례를 하나만 들어주세요.

**F4-2.** 전역 Rate Limit Tracking을 단일 인스턴스 메모리 Map으로 둔 한계가 분명한데, 다중 인스턴스로 확장한다면 어떤 저장소·구조를 택할 건가요?

**F4-3.** Gemini Context Caching의 5분 만료를 운영 동선과 어떻게 맞췄나요? cache miss가 발생하는 패턴이 보였나요?

**F4-4.** 이 12일 경험을 '백엔드 면접관 입장'에서 가장 의심해볼 부분은 어디라고 생각하시나요? 어떻게 답할 준비가 되어 있나요?

**F4-5.** 이 하네스를 커머스 백엔드 일상 업무에 옮긴다면 (예: 쿼리 튜닝 / 장애 대응 / ADR 정리) 어디를 가장 먼저 적용할 수 있을 것 같나요?

---

## 최종 준비 체크리스트

- 면접 직전 30분 동안 4대 핵심 경험(RAG 배치 11 Step / gRPC 503 graceful shutdown / 다중 서버 캐시 정합성 / 12일 AI 웹툰 MVP) 각 1분 요약을 입으로 한 번씩 리허설했는가 — 키워드는 외우지 말고 흐름만 잡는다.
- 각 경험에서 '왜 그 결정을 했는가(트레이드오프)'를 답할 수 있는가 — 예: 왜 JobExecutionContext가 아니라 @JobScope, 왜 30초 재시도가 아니라 즉시 fallback, 왜 Redis가 아니라 인메모리, 왜 SSE가 아니라 Promise.allSettled.
- 기술 블로그 4편(MSA 데이터 연동 / OAuth2 무중단 / SQS 데드락 / Spring 트랜잭션 동기화) 핵심 용어를 본인 경험과 매핑할 수 있는가 — 특히 Cache-Aside·Kafka 이벤트 드리븐·Resilience4j·Feature Flag.
...
- 약점(커머스 도메인 미경험, Kotlin·DB 심화 미흡)에 대해 회피하지 않고 '현재 어떻게 학습 중인지' 구체적 행동으로 답할 수 있는가 — 모호한 다짐이 아니라 베이스라인 학습 트랙 + ADR 정리 습관을 근거로 든다.
