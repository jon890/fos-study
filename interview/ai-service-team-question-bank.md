...
- 팀에서 운영하던 Kafka 이벤트 드리븐(커밋 후 이벤트 발행 기반 비동기 처리) 구조를 접한 경험은, 올리브영의 '무중단 OAuth2 전환'에서 본 Resilience4j 3단계 보호 + Feature Flag + Shadow Mode 같은 운영 안정성 설계와 같은 결의 사고방식이다.
...
### 왜 올리브영인가
- 올리브영 기술 블로그의 'MSA 데이터 연동 전략' 글에서 데이터의 변경 빈도·라이프사이클을 기준으로 Cache-Aside와 Kafka 이벤트 드리븐을 하이브리드로 묶은 부분이, 내가 스포츠 베팅 백엔드에서 RabbitMQ Fanout + StampedLock으로 풀었던 다중 서버 캐시 정합성 문제와 같은 결의 의사결정이라 강하게 공감됐고, 같은 의사결정 패턴을 1,600만 트래픽 규모에서 검증하고 싶다.
...
- 스포츠 베팅 백엔드에서 정적 설정 데이터를 애플리케이션 메모리에 캐싱하면서 어드민 변경을 다중 서버에 반영해야 했고, Hibernate PostCommitUpdateEventListener가 발동하면 RabbitMQ Fanout Exchange로 변경된 데이터 타입 식별자(FANOUT_STATIC_DATA)를 발행하는 구조로 정합성을 유지했다 — 각 서버 인스턴스는 자기 큐에서 메시지를 받아 해당 타입의 캐시만 선택적으로 갱신한다.
...
- 스포츠 베팅 백엔드에서 정적 설정 데이터를 인메모리 캐시로 두고 어드민 변경 시 Hibernate PostCommitUpdateEventListener에서 RabbitMQ Fanout Exchange로 변경된 데이터 타입 식별자를 발행, 각 서버가 자기 큐에서 받아 해당 타입의 캐시만 선택적으로 refresh하는 구조로 다중 서버 정합성을 유지했다.
