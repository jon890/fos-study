...
- Spring AI 기준으로 보면 텍스트 -> 벡터(숫자 배열)로 변환해주는 클라이언트
- Spring에서 벡터 생성 과정을 추상화한 인터페이스
  - 내부적으로는 :
    - WebClient로 OpenAI Embedding API 호출
    - 모델이 텍스트를 분해(tokenize)
    - 의미적 특징을 추출하고
    - 1536차원의 float 배열로 변환하여 반환
- Embedding 모델 동작 원리 (내부적으로)
  - Tokenization
    - 텍스트를 토큰(단어/서브워드)으로 나눔
  - Transformer Encoder
    - BERT, GPT Encoder처럼 self-attention을 기반으로
    - 문장의 의미 구조를 분석
    - 단어 간 관계, 문맥 정보를 모두 포함
  - Pooling(집계)
    - 문장 전체 벡터를 만들기 위해 token embedding를 평균/가중 등으로 합성
  - 정규화(normalization)
    - 검색을 쉽게 하기 위해 L2-normalize 수행하는 경우가 많음
...
### 약관에 대한 문서가 여러 개로 분리되어있다면, 문서를 1개로 통합한 후에, EmbeddingClient를 호출해야 의미있는 벡터가 만들어질까?

- 결론은 X
  - 오히려 성능이 떨어진다
  - Embedding 모델은 **짧고 의미적 단위가 명확한 텍스트**를 가장 잘 벡터화함
  - 긴 문서를 통째로 넣으면 의미가 희석되고 정보 손실이 크게 발생함
  - 검색할 떄도 정확히 매칭이 안 도미 -> Recall/Precision 둘 다 손해
- 올바른 방식
  - 문서를 통합하지 않고, "원문 문서 여러 개를 개별 단위로 처리한 후 -> chunking -> embedding"이 정석
  - 즉:
    1. 문서 별로 텍스트 추출
    2. 문서별로 chunk 분할
    3. 각 chunk에 embedding 생성
    4. VectorStore에 개별로 저장
    5. 검색은 "전체 벡터 공간"에서 수행
- 왜 문서를 합치면 안 되는가?
  - Embedding 모델은 입력 길이가 커질수록 품질이 저하된다
  - OpenAI embedding 모델도 context window가 있음
  - 문서가 길어질수록 attention이 분산됨
