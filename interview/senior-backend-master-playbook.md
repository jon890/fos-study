...
- 보완: 최근 기술 블로그의 MSA 데이터 연동 전략, 무중단 OAuth2 전환(Feature Flag + Shadow Mode + Resilience4j 3단 보호 + ±30s Jitter로 Peak TPS 40% 감소) 글을 정독하며, 슬롯에서 푼 캐시 정합성 문제가 커머스 상품·전시 도메인에 어떻게 매핑되는지 역산 중입니다. 관련 개념을 개인 블로그에 사전 학습으로 정리해뒀습니다:
...
- 보완: 커머스의 실시간 배포 + 무중단 요구가 다른 차원의 문제임을 인지하고, **무중단 OAuth2 전환기**(Feature Flag + Shadow Mode + Resilience4j 3단 + Jitter) 사례를 정독하며 [대규모 트래픽 중 무중단 마이그레이션](../architecture/zero-downtime-migration.md)에 스터디 기록을 남겼습니다. 입사 후 1개월 내 실 사례에 체득하는 것이 목표입니다.
...
> 지난 4년간 NHN에서 **동시성·이벤트 드리븐·대용량 배치** 문제를 깊게 풀어오면서, 같은 패턴이 커머스 도메인에서 더 큰 스케일로 어떻게 다시 나타나는지에 대한 호기심이 계속 쌓였습니다. 슬롯 도메인에서 다중 서버 캐시 정합성을 RabbitMQ + StampedLock으로 풀고, 대용량 RAG 배치 파이프라인을 처음부터 설계한 경험이, **1,600만 고객 트래픽의 상품·전시·주문 도메인**에서 어떤 형태로 다시 등장하는지 직접 보고 싶습니다. 특히 기술 블로그의 MSA 데이터 연동 전략과 무중단 OAuth2 전환기를 읽으면서 **판단의 결이 저와 같다**는 확신이 들었습니다. 여기에 AI 도구를 단순히 쓰는 게 아니라 파이프라인으로 설계·운영한 경험을 더해, 팀의 개발 사이클을 한 단계 당기는 데 기여하고 싶습니다.
...
- **검증 필요성**: 슬롯은 사내·B2C 도메인, AI 서비스는 사내 도구 중심이었습니다. 지금까지의 패턴이 **초 단위 수만 TPS 실서비스 트래픽**에서 얼마나 버티는지는 다음 단계에서 검증해야 합니다. 그 검증 환경으로 대규모 커머스 플랫폼이 가장 적합하다고 판단했습니다.
...
- 접근:
  ...
  4. **Shadow 이행** (구 → 신 병기 기록) 단계로 운영 데이터와 이행 데이터를 비교한 뒤, Feature Flag로 읽기 경로만 점진 전환(무중단 OAuth2 전환 사례와 같은 결).
...
- **커머스 환경의 필요성 인지**: 1,600만 고객 + 실시간 배포 환경에선 다른 차원의 전환 전략이 필요하다는 걸 알고 있고, 무중단 OAuth2 전환기(Feature Flag + Shadow Mode + Resilience4j 3단 + Jitter)를 정독하며 [대규모 트래픽 중 무중단 마이그레이션](../architecture/zero-downtime-migration.md)에 스터디 기록을 남겼습니다.
- **커머스에서 쓸 접근 순서** (도입 시):
...
1. 이 조직이 향후 1\~2년 안에 **가장 투자하려는 기술 영역**(예: 검색 고도화, 추천, 결제, 물류 연계 등)은 무엇인가요?
...
- [ ] 참고 원페이저 1장(이 문서의 3번 항목 커리어 요약 + 8번 항목 프로젝트 요약 + 11번 항목 역질문).
