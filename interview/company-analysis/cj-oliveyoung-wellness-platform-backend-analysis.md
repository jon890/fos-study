...
> "올리브영 웰니스 백엔드는 도메인별로 동기화 전략을 다르게 고르고([A](https://oliveyoung.tech/2026-03-18/oy-store-data-interconnection-strategy/)), 레거시 정합성은 트랜잭션 훅으로 일단 잡되 Outbox로 진화시킬 줄 알며([B](https://oliveyoung.tech/2026-02-23/from-legacy-to-modern-architecture-journey/)), 기반 시스템은 Feature Flag + Shadow + Resilience4j로 점진 전환하고([C](https://oliveyoung.tech/2025-10-28/oliveyoung-zero-downtime-oauth2-migration/)), 그 모든 임계값을 카오스 실험으로 튜닝하는([D](https://oliveyoung.tech/2026-03-30/chaos-host-level/)) 팀이다."
...
- **검색 엔진 연동**: 공고에 "검색 엔진 연동"이 명시돼 있다. 별도 검색 인프라(Elasticsearch 계열)와 상품 색인 동기화 파이프라인이 존재할 가능성이 높다. 카오스 실험 대상에도 "검색"이 들어 있는 점이 이를 뒷받침한다([블로그 D](https://oliveyoung.tech/2026-03-30/chaos-host-level/)).
- **인증/세션 관리**: Spring Authorization Server와 Spring Session이 스택에 포함된다. OAuth2 기반 인증 서버를 자체 운영 중이고, 기술 블로그에서 대규모 트래픽 중 [무중단 OAuth2 전환 경험](https://oliveyoung.tech/2025-10-28/oliveyoung-zero-downtime-oauth2-migration/)을 공개했다(0장 C 참고).
- **알림 시스템**: SQS / Kafka 기반 알림톡 파이프라인이 별도 서비스로 운영되고 있다. [데드락 분석](https://oliveyoung.tech/2025-05-02/oliveryoung-alimtalk-deadlock/)과 [레거시 트랜잭션 동기화 현대화 사례](https://oliveyoung.tech/2026-02-23/from-legacy-to-modern-architecture-journey/)까지 공개돼 있어, 실제로 운영 문제를 직접 풀어 본 영역이다.
- **올영매장 데이터 연동**: 오프라인 매장 도메인 데이터(매장 정보, 재고 상태, 매장 분류 등)를 온라인몰 백엔드가 가져다 쓰는 인터페이스 — [A번 글](https://oliveyoung.tech/2026-03-18/oy-store-data-interconnection-strategy/)에서 본 도메인별 동기화 분기의 실제 무대.
...
### 2-1. 서비스 간 데이터 동기화 설계 ([블로그 A](https://oliveyoung.tech/2026-03-18/oy-store-data-interconnection-strategy/) 직접 매칭)
...
### 2-2. 대규모 트래픽 대응 및 안정성 확보 ([블로그 C](https://oliveyoung.tech/2025-10-28/oliveyoung-zero-downtime-oauth2-migration/) 직접 매칭)
...
### 2-3. 레거시 시스템 현대화 ([블로그 B](https://oliveyoung.tech/2026-02-23/from-legacy-to-modern-architecture-journey/) 직접 매칭)
...
### 2-4. 운영 회복력 — 카오스 엔지니어링 기반 임계값 튜닝 ([블로그 D](https://oliveyoung.tech/2026-03-30/chaos-host-level/) 직접 매칭)
...
10. **MSA 분산 트랜잭션 패턴**: Saga (Choreography vs Orchestration), 2PC 한계, Outbox 상세 (테이블 스키마, Relay 방식, CDC 활용 차이). 참고: [SQS 알림톡 데드락 분석](https://oliveyoung.tech/2025-05-02/oliveryoung-alimtalk-deadlock/) — 비동기 전환 시 트랜잭션 경계 변화가 실제로 어떻게 문제를 만드는지.
...
| 기호 | 주제 | 링크 |
|------|------|------|
| A | 올영매장 데이터 연동 전략 (도메인별 동기화 패턴 분기) | https://oliveyoung.tech/2026-03-18/oy-store-data-interconnection-strategy/ |
| B | 레거시에서 모던 아키텍처로의 여정 (`afterCommit()` 기반 알림톡 현대화) | https://oliveyoung.tech/2026-02-23/from-legacy-to-modern-architecture-journey/ |
| C | 무중단 OAuth2 마이그레이션 (Feature Flag + Shadow + Resilience4j) | https://oliveyoung.tech/2025-10-28/oliveyoung-zero-downtime-oauth2-migration/ |
| D | Host-level 카오스 엔지니어링 (임계값·TTL 회복 SLA 실측) | https://oliveyoung.tech/2026-03-30/chaos-host-level/ |
| 보너스 | SQS 알림톡 데드락 분석 (B번 글의 전사) | https://oliveyoung.tech/2025-05-02/oliveryoung-alimtalk-deadlock/ |
