...
>
> **후속 문서**:
> - [Part 2 — ivips.co.kr SRE 외부 진단](cj-foodville-digital-channel-backend-part2-ivips-sre.md) — 2026-05-18 사전 커피챗 어필용 빕스 공식 사이트 7개 축 외부 진단.
> - [커피챗 준비](cj-foodville-digital-channel-backend-coffeechat.md) — IT 담당자/파트 리드와의 30분 커피챗 대화 흐름, 질문, 답변 프레임.
>
> **관련 면접 문서**:
...
## 한 줄 결론
필수 기술 스택(Java/Spring/JPA/MySQL/클라우드/4년+)은 **전부 충족**하지만, 도메인(F&B·e-Commerce)·일부 보조 스택(Kotlin·MyBatis·JSP·AWS)·아키텍처 키워드(Hexagonal/Clean)에서 갭이 존재. **"운영 안정성·성능·캐시 정합성"을 강점으로 앞세우면 합격권**.
...
| **지원 우선순위** | **A**(메인) | 기술 매칭은 합격권. F&B 도메인 갭은 학습·번역으로 메우는 방향. |
...
19. "왜 CJ푸드빌인가?" — **이게 가장 어려운 질문**이 될 것. 도메인 매칭이 약한 만큼 진정성 답변 준비 필수.
...
3. **"왜 CJ푸드빌인가" 답변 준비**: 게임/AI 도메인에서 F&B 로 옮기는 동기를 **"운영형 자사 서비스 백엔드로의 일관된 커리어"** 라는 큰 그림으로 통합. F&B 는 "결제·주문·매장 운영이라는 숫자가 살아있는 도메인이라 매력적" 이라는 톤 추천.
...
6. **준비 일정 관리**: 사전 커피챗 (2026-05-18) → 정식 1·2차 면접 → 처우 협상 순. **커피챗 인상은 정식 면접에 그대로 carry over** 되므로 같은 무게감으로 준비. 1차 면접 통과 후 처우 협상 단계에서 통근 거리·기술 비중·계약 조건 재확인.
7. **현실 체크**: 연봉 협상 / 출퇴근지(필동/CJ타운) 거리 / 사용 기술 스택 비중을 정식 면접 또는 채용 담당자 통화에서 반드시 확인. **MyBatis·JSP 비중이 절반 이상**이면 합류 후 만족도가 낮을 가능성.
