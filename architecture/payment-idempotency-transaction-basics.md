...
여러 외식 브랜드를 운영하는 다브랜드/다채널 F&B 회사에서 이 문제는 더 까다롭다. 키오스크, 모바일 앱, 배달 플랫폼 연동, 매장 POS가 같은 결제 백엔드를 호출하기 때문에 클라이언트별 재시도 패턴이 다르고, 환불/부분취소/주문 변경 시나리오가 자주 발생한다. 한 건의 중복 결제는 컴플레인 한 통으로 끝나지만, 같은 패턴이 점심 피크 30분 동안 누적되면 정산팀과 CS팀이 며칠을 야근해야 한다.

이 문서는 그 사슬을 안전하게 만드는 가장 기초적인 도구 세 가지에 집중한다. **idempotency key, 주문-결제 상태 분리, Outbox 기반 외부 호출**이다. Saga나 분산 트랜잭션 같은 더 큰 그림도 결국 이 셋 위에서 설명할 수 있어야 설득력이 생긴다.
...
각 단계는 idempotency key를 들고 흐른다. 보상이 필요한 경우 (예: 적립 실패가 정책상 결제 취소를 유발한다면) `PaymentCancelRequested` 이벤트를 역방향으로 흘려보낸다. Saga 설계의 핵심은 "이벤트 한 줄 한 줄이 idempotent해야 보상이 안전하다"는 점이다. 그렇지 않으면 보상이 또 다른 사고를 만든다.

## 부분취소와 환불의 함정

F&B 환경에선 "샐러드 한 개만 취소"가 일상이다. 부분취소는 결제 측면에서 두 가지 변형이 있다.
...
이렇게 하면 누적 환불 금액 = `SUM(amount WHERE type=REFUND AND status=SUCCEEDED)` 로 단순 집계되고, 감사로그가 자연스럽게 시계열로 정렬된다. 부분취소를 모델링할 때 이 구조를 기본으로 잡으면 사후 추적이 단순해진다.
...
각 시나리오에서 `payment_audit`가 어떻게 쌓이는지 직접 본다. 이 경험이 설계 디테일을 잡는 감각을 만든다.

## 결제 중복 방지를 설명하는 순서

"결제 중복을 어떻게 막는가"를 설명할 때는 다음 순서로 풀면 빠짐이 없다.

1. **레이어를 분리한다**: 클라이언트 idempotency key, 서버 idempotency 테이블, payments 테이블 unique constraint 세 단계.
2. **타임아웃 처리를 함께 본다**: 타임아웃은 실패가 아니라 미상이라는 점, PENDING 상태로 두고 reconciliation에 위임한다는 점.
...
5. **한계를 인정한다**: 완벽한 분산 트랜잭션은 없고, 감사로그와 재처리로 최종 일관성을 보장한다.

"트랜잭션 안에서 외부 호출을 하면 안 되는 이유"는 두 가지를 묶어 설명한다. 첫째, 커넥션 풀이 외부 응답시간만큼 잡혀 동시성이 깨진다. 둘째, commit과 외부 호출의 원자성을 보장할 수 없어서 어차피 실패 시나리오가 남는다.

Saga는 프레임워크 이름이 아니라 Outbox + 이벤트 기반 보상의 골격으로 이해하는 편이 본질에 가깝다. 보상 가능한 이벤트 설계 원칙이 핵심이다.
...
- [ ] 결제 중복 방지를 위 5단계로 압축해서 설명할 수 있는가

## 관련

- [커머스 주문 상태와 데이터 정합성 기본기](./commerce-order-state-consistency-fundamentals.md) — 주문 상태 전이와 결제 멱등성이 만나는 지점

## 2026-05-19 보강 — PG 승인·취소·대사 운영

F&B 결제 설계에서는 “승인 성공/주문 실패”와 “주문 성공/응답 유실”을 반드시 분리해서 다뤄야 한다. 사용자는 같은 버튼을 다시 누르지만 서버는 이전 결제 시도와 같은 의도인지 판단해야 하므로, 결제 요청에는 `payment_attempt_id` 또는 서버 발급 idempotency key가 필요하다.

운영 설계 체크리스트는 다음과 같다.

- 결제 요청 저장을 PG 호출보다 먼저 수행하고, unique key로 동일 시도를 한 번만 처리한다.
- PG 응답을 받지 못한 timeout은 즉시 실패 확정하지 않고 `UNKNOWN` 또는 `VERIFY_REQUIRED` 상태로 둔다.
- webhook, client return, 운영 대사 배치가 같은 결제 row를 갱신할 수 있으므로 상태 전이는 단조롭게 설계한다.
- 부분취소는 원 승인 row를 덮어쓰지 말고 refund transaction row를 별도로 남긴다.
- 환불 실패는 주문 취소 실패와 동일하지 않다. 주문은 취소됐지만 환불 보상이 필요한 상태를 표현해야 한다.
- 대사 배치는 PG 거래번호, 주문번호, 금액, 승인시각, 취소시각을 기준으로 “우리 DB 없음 / PG 없음 / 금액 불일치 / 상태 불일치”를 분리한다.

한 줄 요약은 “결제 외부 호출은 DB 트랜잭션 안에서 길게 잡지 않고, 요청 의도와 상태를 먼저 저장한 뒤 PG 호출 결과를 멱등하게 반영한다. 응답 유실은 조회/대사로 수렴시키고, 환불과 부분취소는 별도 트랜잭션으로 추적해 CS 복구가 가능하게 한다”이다.
