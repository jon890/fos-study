...
## 2026-05-19 CJ푸드빌 부트캠프 보강 — PG 승인·취소·대사 운영

F&B 결제 면접에서는 “승인 성공/주문 실패”와 “주문 성공/응답 유실”을 반드시 분리해서 말해야 한다. 사용자는 같은 버튼을 다시 누르지만 서버는 이전 결제 시도와 같은 의도인지 판단해야 하므로, 결제 요청에는 `payment_attempt_id` 또는 서버 발급 idempotency key가 필요하다.
...
면접 답변 예시는 “결제 외부 호출은 DB 트랜잭션 안에서 길게 잡지 않고, 요청 의도와 상태를 먼저 저장한 뒤 PG 호출 결과를 멱등하게 반영합니다. 응답 유실은 조회/대사로 수렴시키고, 환불과 부분취소는 별도 트랜잭션으로 추적해 CS 복구가 가능하게 합니다”가 좋다.
