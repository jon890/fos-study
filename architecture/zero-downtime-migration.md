...
```mermaid
flowchart TD
    REQ["요청 수신"] --> PRIM["레거시 경로 실행<br/>(사용자에게 이 응답 반환)"]
    PRIM --> RESP["응답 반환"]
    PRIM --> SHADOW["별도 스레드풀에서<br/>신규 경로 비동기 실행"]
    SHADOW --> DIFF{"결과 일치?"}
    DIFF -->|"일치"| OK2["match 카운터 증가"]
    DIFF -->|"불일치"| WARN["diff 로그 + 카운터 증가"]
...
실전 순서는 보통 이렇다. **Shadow Mode**(로직 검증) → **Canary 1% → 5% → 25%**(실사용 부하 관찰) → **50% → 100%**(전환 완료) → **Legacy 제거**.

```mermaid
flowchart LR
    S1["Shadow Mode<br/>로직 동등성 검증"] --> S2["Canary 1%<br/>실트래픽 소량"]
    S2 --> S3["Canary 25%<br/>성능·오류율 관찰"]
    S3 --> S4["100% 전환"]
    S4 --> S5["Legacy 제거<br/>플래그 cleanup"]

    S2 -->|"문제 발생"| ROLL["즉시 Flag OFF<br/>레거시로 복귀"]
    S3 -->|"문제 발생"| ROLL
``` Blue/Green은 인프라 여유가 있는 조직(특히 쿠버네티스에서 두 deployment를 동시에 유지 가능한 경우)에서 빠른 스위치가 필요할 때 선택한다. OAuth2 같이 **세션/토큰 상태 호환성**이 핵심인 전환은 대부분 Shadow → Canary 순서로 간다. Blue/Green은 토큰 호환 구간을 다루기 까다롭다.
