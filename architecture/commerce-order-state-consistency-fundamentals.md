...
외식 프랜차이즈처럼 매장 POS, 키오스크, 모바일 앱, 배달 플랫폼이 동시에 같은 주문 도메인을 건드리는 환경에서는 다음 세 가지가 시니어 백엔드의 기본기로 요구된다.
...
| 멤버십 | 적립은 `COMPLETED` 시점에만 1회 (`event_id` dedup) | 같은 주문에 멤버십 포인트 2배 적립 |

여기서 중요한 것은 **"어디에 어떤 책임을 두는지"** 를 명확히 정하는 것이다. 같은 불변 조건이 여러 층에 흩어지면 우회 경로로 깨지고, 한 층에만 두면 운영 환경에서 우회당한다. 다층 방어가 정석이다.
...
"그럼 SERIALIZABLE을 쓰면 되지 않냐"는 의문이 따라올 수 있다. 정리하면 이렇다.
...
업데이트 row 수가 0이면 "이미 다른 상태로 바뀌었다"고 판단해 비즈니스 예외를 던진다. **단순 전이는 이 한 줄로 대부분 안전하게 처리된다**.
...
"Outbox를 어떻게 발행하느냐"는 단순 "스케줄러가 polling 한다"로 끝나지 않는다. 실제로 운영하면 다음 3가지가 문제다.
...
"왜 `@TransactionalEventListener(AFTER_COMMIT)`인가?"를 정리하면 이렇다.
...
운영 사고는 "잘못된 상태가 이미 DB에 박혀서 트래픽을 받는 중"이라는 시점부터 시작된다. 코드 수정만으로 끝나지 않고 **이미 잘못된 row**를 어떻게 되돌릴지가 본격적인 일이다. "운영 사고를 어떻게 복구하느냐"는 사실상 이 플레이북을 묻는 것이다.
...
이 표를 몸에 익혀 두면 사고 대응이 빨라진다. "사고 났을 때 첫 5분에 뭘 하느냐"의 핵심은 **트래픽 차단이 코드 수정보다 먼저**라는 점이다.
...
## 설계 요약 프레이밍 (시니어 백엔드 톤)

### "주문 처리 시스템을 어떻게 설계하는가"
...
### "주문 상태가 잘못된 채로 운영에 나갔을 때 어떻게 복구하는가"
...
## 실무 경험을 주문 도메인 언어로 번역하기

추상적인 원칙만 두지 말고, 실제 구현 경험을 주문 도메인 용어로 옮겨 두면 같은 설계를 다른 도메인에 그대로 이식할 수 있다.

### Kafka Transactional Outbox 운영 경험

> read-heavy 캐시를 다루는 핵심 API의 동기/비동기 후처리를 분리할 때 Kafka 발행이 DB 커밋 전에 나가는 사고를 막기 위해 `@TransactionalEventListener(AFTER_COMMIT)` 기반 Outbox를 깔았다. 발행 실패 시 `Propagation.REQUIRES_NEW`로 실패 메시지를 별도 트랜잭션에 저장하고 스케줄러가 재발행하는 구조였고, traceId를 메시지·실패 테이블에 같이 박아 사후 추적이 가능하게 했다. 주문 도메인으로 옮기면 그대로 "주문 상태 변경 + outbox 적재가 같은 트랜잭션, POS/알림/적립은 별도 publisher" 구조로 매칭된다.

### RabbitMQ Fanout 다중 서버 캐시 정합성 경험

> 정적 마스터 데이터를 다중 서버 인메모리에 캐싱하던 환경에서, 어드민 수정 시 서버 간 정합성이 깨지고 갱신 중 NPE가 났다. Hibernate `PostCommitUpdateEventListener` → RabbitMQ Fanout으로 전 서버에 무효화 신호를 뿌리고, 갱신 구간은 `StampedLock` writeLock으로 보호, 조회는 `tryReadLock(2.5s)`로 막았다. 주문 도메인의 매장/메뉴/프로모션 정책 캐시도 같은 패턴으로 풀린다.

### StampedLock 정적 데이터 동시성 경험

> 갱신 빈도는 낮고 조회가 압도적인 정적 마스터 데이터에서 `synchronized`나 `ReentrantReadWriteLock`로는 reader가 너무 자주 막혔다. `StampedLock` + optimistic read로 갱신 없을 때는 락 없이 흐르게 만들었고, writer 진입 시점에만 `tryWriteLock`/타임아웃을 박았다. 주문 라우팅에서 매장 메타데이터를 매 요청 조회한다면 동일한 패턴이 적합하다.
...
- [ ] 설계 요약 6가지 골격을 막힘없이 설명할 수 있는가
- [ ] Outbox / RabbitMQ Fanout / StampedLock 경험을 주문 도메인 언어로 옮길 수 있는가
- [ ] 시간순 대응 체크리스트(0\~5분 트래픽 차단 / 30분 영향 집합 / 1\~2시간 샘플 보정)를 숙지하고 있는가
...
## 2026-05-19 보강 — 주문 정합성 정리 프레임

주문 정합성을 설명할 때는 “상태 전이 + 멱등키 + 락/격리수준 + 이벤트 발행 + 복구”를 한 문장으로 묶어 정리한다. 예를 들어 “모바일 앱 재시도로 동일 주문 요청이 여러 번 들어와도 `order_request_id` unique key로 한 번만 생성하고, 상태 전이는 현재 상태를 조건으로 둔 compare-and-set update로 막으며, 결제 성공 후 POS 접수 이벤트는 Outbox에 남겨 재처리한다”처럼 정리하면 도메인과 기본기가 함께 드러난다.
...
핵심은 “정합성은 코드 if문이 아니라 DB 제약, 트랜잭션 경계, 상태 전이, 재처리 설계가 같이 보장한다”는 관점이다.
