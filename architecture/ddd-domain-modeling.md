...
# DDD와 도메인 모델링: 시니어 백엔드 관점의 전술/전략 패턴 실전 가이드
...
```mermaid
flowchart TB
    subgraph OrderCtx["Order Bounded Context"]
        OR["Order<br/>(Aggregate Root)"]
        OL["OrderLine<br/>(내부 Entity)"]
        MONEY["Money<br/>(Value Object)"]
        OR -->|"컬렉션 소유"| OL
        OR -->|"포함"| MONEY
    end

    subgraph PayCtx["Payment Bounded Context"]
        PAY["Payment<br/>(Aggregate Root)"]
    end

    subgraph CouponCtx["Coupon Bounded Context"]
        COU["Coupon<br/>(Aggregate Root)"]
    end

    OR -->|"ID 참조만<br/>(객체 참조 X)"| MID["MemberId"]
    OR -->|"ID 참조만"| CID["CouponId"]
    OR -->|"OrderCancelledEvent<br/>이벤트 발행"| PAY
```
