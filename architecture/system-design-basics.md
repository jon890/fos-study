...
이 문서는 "URL shortener를 그려 보세요"라는 고전적 질문에 45분 안에 요구사항 명확화부터 장애 대응까지 매끄럽게 talk-through할 수 있도록 구성된 실전 스터디 팩이다. 대규모 커머스 플랫폼처럼 트래픽 스파이크가 분명하고, 커머스/회원/추천이 동시에 돌아가는 환경에서 시니어로서 설득력 있게 말할 수 있어야 한다.
...
엔진 추상화 경험: "다양한 도메인 모델을 engine abstraction 뒤에 숨겨, 클라이언트 계약을 깨지 않고 엔진을 추가/교체했다"는 서술은 **결합도 분리 / plug-in 아키텍처 / contract-first** 경험으로 연결된다. 시스템 설계 라운드에서 "서비스 분리 기준"이나 "stable interface와 내부 진화를 어떻게 양립시켰는가"를 물을 때 강력한 답변이 된다.

대량 배치 경험: "대량 문서 임베딩 배치를 안정적으로 운영했다"는 서술은 **배치 vs 스트림, 재시도, idempotency, backpressure, partial failure 복구** 경험이다. Kafka + 집계 잡 이야기를 할 때 자연스럽게 "실제로 이런 구조에서 poison message와 DLQ를 어떻게 처리했다"로 이어갈 수 있다.
...
- [ ] 엔진 추상화 / 대량 배치 경험을 "결합도 분리", "배치 vs 스트림", "idempotency" 키워드와 각각 연결할 수 있다.
...
- [캐시 설계 전략 총정리](./cache-strategies.md) — 캐시 패턴 레퍼런스
- [분산 트랜잭션과 Outbox 패턴](./distributed-transaction-outbox-pattern.md) — 2PC 대안
- [분산 아키텍처 스터디 팩](./distributed-architecture-study-pack.md) — 서비스 경계, 일관성, 멱등성
- [대규모 커머스 트래픽 처리 패턴](./high-traffic-commerce-patterns.md) — 구체 사례
