...
### 타임아웃 후 부분 성공 (Partial Success After Timeout)
...
sequenceDiagram
    ...
    I->>I: 재고 차감
    I-->>P: InventoryReserved 이벤트

    P->>P: 결제 처리
    P-->>D: PaymentProcessed 이벤트

    D->>D: 배송 준비
    D-->>O: OrderCompleted 이벤트

    alt 결제 실패 시 보상
        P-->>I: InventoryReleased 이벤트
        P-->>O: 주문 취소 이벤트
        I->>I: 재고 복구
        O->>O: 주문 취소
    end
...
```mermaid
flowchart LR
    CLOSED["CLOSED<br/>(정상 처리)"]
    OPEN["OPEN<br/>(요청 차단)"]
    HALF["HALF_OPEN<br/>(탐색 중)"]
