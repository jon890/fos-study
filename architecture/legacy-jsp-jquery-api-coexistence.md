...
```mermaid
flowchart TD
    GW["요청 진입점<br/>(nginx / API Gateway)"] --> CHECK{URL 패턴}
    CHECK -->|"/api/v1/**"| APICHAIN["Spring Security 체인 1<br/>JWT 인증 (STATELESS)"]
    CHECK -->|"그 외"| LEGCHAIN["Spring Security 체인 2<br/>세션 인증"]
    APICHAIN --> APICTRL["REST 컨트롤러<br/>JSON 응답 규약"]
    LEGCHAIN --> JSPCTRL["MVC 컨트롤러<br/>JSP 렌더링 / legacy ajax"]
    APICTRL --> SVC["도메인 서비스 (공유)"]
    JSPCTRL --> SVC
```
...
```mermaid
flowchart LR
    subgraph Phase1["1단계: 전체 레거시"]
        A1["모든 요청 → Tomcat (JSP)"]
    end
    subgraph Phase2["2단계: 일부 절단"]
        B1["/order/new → SPA"]
        B2["나머지 → Tomcat"]
    end
    subgraph Phase3["3단계: 점진 확장"]
        C1["/order/** → SPA"]
        C2["/coupon/** → SPA"]
        C3["나머지 → Tomcat"]
    end
    subgraph Phase4["4단계: 레거시 제거"]
        D1["모든 요청 → SPA / 신규 API"]
    end
