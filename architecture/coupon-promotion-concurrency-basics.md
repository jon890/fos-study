...
CJ푸드빌처럼 이벤트 오픈 피크가 짧고 강한 곳이라면 전략 3이 사실상 표준이다.
...
## 2026-05-19 CJ푸드빌 부트캠프 보강 — 선착순 이벤트와 CS 복구

쿠폰/프로모션 답변은 “빠른 발급”보다 “초과 발급과 중복 사용을 어떻게 끝까지 막는가”가 더 중요하다. Redis counter만으로 성공을 확정하면 장애 복구 시 RDBMS와 불일치가 생기기 쉽고, RDBMS 락만으로 처리하면 이벤트 오픈 피크에 병목이 된다.
...
면접에서는 “Redis로 빠르게 줄 세우되, 최종 정합성은 RDBMS 제약과 상태 전이가 보장하게 설계합니다. 장애 시에는 발급/사용/복구 이벤트를 감사로그로 남겨 CS가 어떤 주문에서 쿠폰이 묶였는지 추적할 수 있게 합니다”라고 정리하면 좋다.
