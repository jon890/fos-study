...
F&B 계열 e-Commerce 백엔드는 단순한 "장바구니 → 결제" 흐름이 아니다. 같은 주문 한 건이 매장 픽업, 배달, 예약, 쿠폰 결합, 멤버십 적립, 프로모션 가격 정책, 결제 승인/부분취소/환불, 재고 차감, 매장 운영시간 검증까지 동시에 만족해야 한다. 도메인이 잘못 잘리면 한 가지 실패 시나리오만 발생해도 "결제는 됐는데 주문은 안 들어간다", "쿠폰은 차감됐는데 결제는 취소됐다", "재고는 빠졌는데 매장에서 거절했다" 같은 운영 사고가 그대로 사용자에게 노출된다.

이 주제는 단일 모델 설계 문제가 아니라 **분산 환경에서 정합성을 어떻게 지키는가**, **장애가 났을 때 시스템이 어떻게 회복하는가**, **운영 관점에서 무엇을 지표로 보는가**까지 이어진다. 따라서 본 문서는 도메인을 어떻게 자르는지, 상태머신을 어떻게 정의하는지, 멱등성과 Outbox/Saga를 언제 어떻게 적용하는지를 한 흐름으로 정리한다.
...
stateDiagram-v2
    [*] --> CREATED : 주문 객체 생성
    CREATED --> PAYMENT_PENDING : 결제 진입
    PAYMENT_PENDING --> PAYMENT_APPROVED : PG 승인
    PAYMENT_PENDING --> PAYMENT_FAILED : PG 실패
    PAYMENT_PENDING --> CANCELED_BY_USER : 사용자 취소
    PAYMENT_APPROVED --> STORE_ACCEPTED : 매장 수락
    PAYMENT_APPROVED --> STORE_REJECTED : 매장 거절
    PAYMENT_APPROVED --> CANCELED_BY_USER : 사용자 취소
    STORE_ACCEPTED --> PREPARING : 조리 시작
    PREPARING --> "READY_FOR_PICKUP" : 준비 완료
    "READY_FOR_PICKUP" --> COMPLETED : 수령 완료
    STORE_REJECTED --> REFUND_REQUESTED : 자동 환불 트리거
    CANCELED_BY_USER --> REFUND_REQUESTED : 환불 요청
    REFUND_REQUESTED --> REFUNDED : PG 환불 완료
    COMPLETED --> [*]
    REFUNDED --> [*]
    PAYMENT_FAILED --> [*]
```
...
서비스 경계가 여러 개라면 분산 트랜잭션 대신 **[Outbox 패턴](distributed-transaction-outbox-pattern.md) + 보상 트랜잭션**(Saga) 조합을 쓴다. 정합성 설계에서 가장 핵심이 되는 부분이다.
...
sequenceDiagram
    ...
    participant Store as 매장 알림 서비스
    participant Payment as 결제 서비스
    participant PG as PG
    participant Coupon as 쿠폰 서비스

    Order->>Store: OrderPaymentApprovedEvent (Kafka)
    Store->>Store: 매장 POS에 주문 전달
    Store-->>Order: StoreRejectedEvent (재료 부족 / 마감)
    Order->>Payment: StoreRejectedEvent (Kafka)
    Payment->>PG: 결제 취소 호출 (Void / Refund)
    PG-->>Payment: 취소 완료
    Payment->>Order: PaymentCanceledEvent (Kafka)
    Order->>Order: CANCELED_BY_STORE 전이
    Order->>Coupon: CouponRestoreEvent (Kafka)
    Coupon->>Coupon: 쿠폰 ISSUED 복구 (멱등)
```
...
## Kafka Outbox·캐시 정합성·장애 대응을 e-Commerce 도메인으로 풀어내기

### 주문/결제 흐름에서 데이터 정합성을 어떻게 지키는가

> 핵심은 분산 트랜잭션을 만들지 않는 것이다. 도메인 변경과 메시지 발행을 같은 RDB 트랜잭션에 묶는 Outbox 패턴을 기본으로 두고, 그 다음 단계는 Saga로 보상한다. 도메인 변경과 외부 시스템 알림이 분리된 상태에서 메시지가 누락되는 사고는 Outbox로 표준화하면 잡힌다. e-Commerce에서는 주문 생성·결제 승인·매장 통보·적립이 각자의 책임이라 이 패턴이 그대로 들어맞는다. 매장 거절은 Saga의 보상 트랜잭션 트리거로 보고, 결제 취소·쿠폰 복구·재고 복구를 멱등 컨슈머로 잇는다.

### 캐시 정합성은 어떻게 다루는가

> 상품 단가, 매장 운영시간, 멤버십 등급 같은 읽기 비중이 큰 데이터를 캐시 대상으로 본다. 다만 주문 시점 가격은 캐시에서 읽되 **주문 객체 안에 스냅샷으로 저장**한다. 정책이 바뀌어도 과거 주문이 흔들리면 안 되기 때문이다. 캐시 갱신은 변경 시점에 publish-then-invalidate로 흘리고, TTL 안전장치를 함께 둬서 invalidate 누락 시에도 자동 회복되게 한다. 캐시 stampede로 결제 직전 단가 조회가 폭주하면 DB 부하가 튀므로, 단건 캐시 + 짧은 TTL + jitter 패턴으로 막는다.

### 결제 PG 장애가 나면 어떻게 복구하는가

> 가장 중요한 건 사용자에게 "결제됐는지 안 됐는지" 모호한 상태를 보여주지 않는 것이다. 그래서 PG 호출 직전에 "승인 요청 중" 레코드를 자체 DB에 먼저 커밋하고, 응답을 못 받았을 때를 대비한 reconciliation job을 따로 운영한다. 이 잡이 PG에 거래 상태를 다시 조회해서 자체 DB와 맞추고, 사용자에게는 "결제 확인 중" 상태로 노출한다. 운영 모니터링은 PG 응답시간 p99, 승인 실패율, reconciliation 보정 건수, outbox lag 네 가지를 1분 단위로 본다. 이 지표 중 하나라도 임계 초과하면 결제 수단별로 트래픽을 일시 분산하거나 매장 알림 큐를 늦추는 식으로 대응한다.
