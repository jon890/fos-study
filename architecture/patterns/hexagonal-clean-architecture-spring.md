...
실무에서도 똑같은 형태로 물음이 던져진다. "도메인 로직과 인프라 코드를 어떻게 분리하는가", "JPA 엔티티를 그대로 도메인 모델로 쓰는 것의 문제는 무엇인가", "이 코드에 트랜잭션 경계를 어디에 두어야 하는가". 이 질문들에 답하려면 Hexagonal/Clean이라는 단어를 외우는 것으로는 부족하고, **경계를 어디에 긋고 무엇을 그 경계 안에 둘지**를 자기 언어로 설명할 수 있어야 한다. 이 글은 그 경계를 Spring 코드 위에 직접 그려 보는 것을 목표로 한다.

도메인 모델 자체에 대한 더 일반적인 설명은 [DDD와 도메인 모델링](../domain/ddd-domain-modeling.md) 문서가 어울리고, 이 문서는 그중에서도 **Hexagonal/Clean이라는 경계 모델을 Spring 컴포넌트(Controller, Service, JPA, Kafka 등) 위에 올리는 실전 기술**에 집중한다.
...
디지털 채널(웹/앱/POS/배달 플랫폼) 다채널을 다루는 백엔드는 같은 "주문" 개념이 채널마다 약간씩 다르게 영속화되거나, 외부 채널 API 응답에 끌려다니기 쉽다. 이때 JPA 엔티티 = 도메인 전략을 끝까지 끌면 채널마다 엔티티를 복제하거나 한 엔티티에 채널 분기가 누적되는 흔한 안티패턴이 생긴다. 도메인 `Order`와 채널별 `OrderProjection` / `OrderJpaEntity`를 분리하는 편이 결국 변경 비용이 싸다.
...
## 다중 Provider 사례를 포트-어댑터로 정리하기

외부 결과를 분석하는 도메인 로직을 포트-어댑터 언어로 다시 풀면 다음과 같이 정리된다. 예로 "분석 템플릿이 정한 규칙으로 외부에서 받은 결과를 산출하는" 유스케이스를 들 수 있다.

> 결과 분석은 분석 템플릿(설정 템플릿)이라는 도메인 개념이 중심이고, 분석기는 그 템플릿이 정한 규칙을 적용해 한 건의 결과를 산출하는 책임이다. 외부에서 보면 '결과 분석'이라는 하나의 유스케이스이고, 안에서는 템플릿 조회와 결과 산출이라는 도메인 협력이다. 이걸 포트-어댑터로 다시 보면, Inbound Port는 `AnalyzeResultUseCase` 같은 형태가 되고, Outbound Port는 분석 템플릿 조회를 위한 `TemplateRepositoryPort`와 외부 Provider별 결과 조회를 위한 `ResultProviderPort`로 분리된다. Provider별로 응답 포맷이 다르면 전략 패턴으로 구현하는데, 그게 곧 한 Outbound Port에 대한 다중 어댑터(ProviderA Adapter / ProviderB Adapter)다. 도메인은 어느 Provider를 쓰는지 모르고, 어댑터 선택은 Application Service나 팩토리가 책임진다. 그래서 새 Provider가 붙어도 도메인과 UseCase는 변경 없이 어댑터만 추가하면 된다.

이 정리에는 핵심 키워드가 자연스럽게 들어간다 — 책임 분리, Port/Adapter, 전략 패턴의 역할, OCP, 변경 영향 범위.

자주 이어지는 후속 질문 정리:
- "JPA 엔티티와 도메인을 분리하는가?" → 단계 1/2 기준 그대로다. 규칙이 자라는 도메인은 분리하고, CRUD에 가까운 부분은 엔티티 재사용으로 둔다. 분리 기준은 영속성 라이프사이클이 도메인 표현력을 망치기 시작하는 시점이다.
- "트랜잭션 경계는?" → Application Service에 두고, 외부 호출은 AFTER_COMMIT 또는 outbox로 미룬다. Controller에는 트랜잭션을 두지 않는다.
- "그게 과설계 아닌가?" → 도입 강도를 세 단계로 보고, 새 유스케이스부터 상위 단계로 시작해 ArchUnit으로 침범을 막는 방식으로 점진 도입한다. 한 번에 전체 리팩터링은 비용 대비 효과가 낮다.
- "Clean Architecture와 Hexagonal Architecture 차이는?" → 본질은 같다. 의존성 방향이 항상 안쪽으로 향한다는 규칙이다. Clean은 동심원으로 표현하고 Use Cases / Entities를 명시적으로 구분하는 반면, Hexagonal은 Port/Adapter 입출력 비유로 표현한다. Spring 위에서는 둘 다 결국 Controller-UseCase-Domain-Port-Adapter라는 같은 다섯 부류로 구현된다.
...
- [ ] "이건 왜 이렇게 했나"를 단계 0/1/2 기준으로 trade-off까지 설명할 수 있다
