...
# 시니어 백엔드를 위한 Resilience 패턴 실전 가이드 — Timeout, Retry, Circuit Breaker, Bulkhead, Backpressure
...
stateDiagram-v2
    [*] --> Closed
    Closed --> Closed : 호출 성공 (실패율 측정 중)
    Closed --> Open : 실패율 >= 임계값 (예: 50%)
    Open --> Open : 모든 호출 즉시 실패 반환
    Open --> HalfOpen : waitDurationInOpenState 경과
    HalfOpen --> Closed : 프로브 호출 성공
    HalfOpen --> Open : 프로브 호출 실패

    note right of Closed : 실패율·느린 호출 비율 측정
    note right of Open : CallNotPermittedException 반환<br/>스레드 대기 없음
    note right of HalfOpen : permittedNumberOfCalls 만큼만 허용
```
...
```mermaid
flowchart LR
    REQ["요청"] --> BH["Bulkhead<br/>(자원 격리)"]
    BH --> TL["TimeLimiter<br/>(전체 시간 제한)"]
    TL --> CB["CircuitBreaker<br/>(회로 상태 확인)"]
    CB -->|"Open이면 즉시 실패"| FAIL["CallNotPermittedException"]
    CB -->|"Closed / Half-Open"| RT["Retry<br/>(재시도 + 백오프)"]
    RT --> CALL["실제 호출"]
    CALL -->|"성공"| OK["응답 반환"]
    CALL -->|"실패"| RT
```
