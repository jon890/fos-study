...
CJ푸드빌처럼 "주문/결제/매장 운영"이 핵심이면 그 영역만 헥사고날로 가져가고 사이드 도메인(공지사항, 운영자 화면 일부)은 굳이 깊이 적용하지 않는다. 이 판단을 **Pragmatic Clean Architecture**라고 부르며, 면접에서 강조해야 할 부분이다.
...
A. application service의 유스케이스 메서드 한 점에만 `@Transactional`을 둡니다. 어댑터에는 걸지 않습니다. 외부 호출(PG, 메시지 발행)이 트랜잭션 안에 들어가면 commit 시점과 외부 효과 시점이 어긋나 정합성이 깨질 수 있어, 가능하면 외부 호출은 트랜잭션 밖이나 outbox 패턴으로 분리합니다.
