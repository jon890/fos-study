...
지표를 두 갈래로 가른다.
이 구분은 공식 문서에 있는 것이 아니라 지표의 종류에서 끌어낸 것이다.
...
llm-d 가 요청 경로에서 읽는 값이 `vllm:kv_cache_usage_perc` 라는 것도 이 구분과 맞는다.
gauge 하나를 replica 마다 읽어 비교하는 것으로 판단이 끝난다.
공개 문서가 이 지표를 고른 이유를 밝히지는 않았다.
...
이 값은 counter 이므로 요청 경로에서 그대로 비교할 수 없다.
`rate()` 로 구간 증가율을 구해 별도 수집 계통에 두고, 그 값이 오르는 인스턴스의 가중치를 정책으로 낮추는 쪽이 맞다.
요청 단위로 판단하려면 같은 상태를 gauge 로 읽을 수 있는 `kv_cache_usage_perc` 를 본다.
