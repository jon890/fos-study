...
필자의 슬롯 엔진(447 테스트) 기준 실제 분포는 단위 60% / slice 22% / 통합 18% 수준이었다. **핫패스는 통합까지 붙여 검증하고, 규칙 계산부는 단위로 빠르게 회전**시키는 원칙이었다.
...
**1) 레이어 결정**
"저는 먼저 도메인 위험도에 따라 피라미드 비율을 조정합니다. 돈이 흐르는 도메인에서는 통합 비중을 25~30%까지 올리고, CRUD 성격 모듈은 슬라이스 테스트 위주로 갑니다. 슬롯 엔진에서는 447개 테스트를 단위 60% / slice 22% / 통합 18% 수준으로 운영했습니다."
