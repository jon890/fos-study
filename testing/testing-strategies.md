...
필자는 실무에서 슬롯 게임 서비스에 **350개의 테스트 파일**을 유지하면서, 슬롯마다 반복되던 테스트 셋업을 **추상 클래스와 JUnit5 Extension으로 뽑아내 팀 누구나 새 슬롯에 테스트를 쉽게 붙일 수 있는 공통 인프라**를 만든 경험이 있다. "새 슬롯은 구현체 하나와 추상 메서드 두 개만 구현하면 통합 테스트가 붙는다"가 목표였다. 이 글은 그 경험에서 나온 결정들을 구조화한 것이다.
...
필자의 슬롯 서비스(테스트 파일 350개) 기준 실제 분포는 **통합 75% / 단위 25% / slice 0%**로, 위의 일반 가이드와 오히려 반대였다. 이유가 있다. 슬롯 도메인은 잭팟 풀·개인화 데이터 같은 **상태 기반 의존성**이 많아 단위 테스트의 Mock 관리 비용이 컸고, 그래서 `AbstractSlotTest`로 Spring 컨텍스트를 띄우는 통합 테스트로 무게를 옮겼다. 또 직접 책임지는 HTTP/JPA 어댑터가 적어 slice를 따로 두지 않고 통합으로 흡수했다. **핫패스는 통합까지 붙여 검증하고, 값 객체·유틸 같은 순수 계산부만 단위로 빠르게 회전**시키는 구조다. 피라미드 비율은 정답값이 아니라 도메인 특성에 따라 뒤집힐 수 있다는 점이 핵심이다.
...
**1) 레이어 결정**
"저는 먼저 도메인 특성에 따라 피라미드 비율을 조정합니다. 상태 기반 의존성이 많고 리그레션이 치명적인 도메인은 통합 비중을 과감히 올립니다. 슬롯 서비스에서는 테스트 파일 350개를 통합 75% / 단위 25% / slice 0%로 운영했습니다. Mock 관리 비용이 커서 통합 위주로 갔고, 직접 책임지는 어댑터가 적어 slice는 통합으로 흡수했습니다. 반대로 CRUD 성격 모듈이라면 slice 위주로 갔을 겁니다."
