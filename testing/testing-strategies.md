...
필자는 실무에서 슬롯 게임 서비스에 **447개의 테스트 파일**을 유지하면서, 슬롯마다 반복되던 테스트 셋업을 **추상 클래스와 JUnit5 Extension으로 뽑아내 팀 누구나 새 슬롯에 테스트를 쉽게 붙일 수 있는 공통 인프라**를 만든 경험이 있다. "새 슬롯은 구현체 하나와 추상 메서드 두 개만 구현하면 통합 테스트가 붙는다"가 목표였다. 이 글은 그 경험에서 나온 결정들을 구조화한 것이다.
...
필자의 슬롯 서비스(테스트 파일 447개, 테스트 클래스 기준) 실제 분포는 **통합 75% / 단위 25% / slice 0%**로, 위의 일반 가이드와 오히려 반대였다. 이유가 있다. 슬롯 도메인은 잭팟 풀·개인화 데이터 같은 **상태 기반 의존성**이 많아 단위 테스트의 Mock 관리 비용이 컸고, 그래서 `AbstractSlotTest`로 Spring 컨텍스트를 띄우는 통합 테스트로 무게를 옮겼다. 또 직접 책임지는 HTTP/JPA 어댑터가 적어 slice를 따로 두지 않고 통합으로 흡수했다. **핫패스는 통합까지 붙여 검증하고, 값 객체·유틸 같은 순수 계산부만 단위로 빠르게 회전**시키는 구조다. 피라미드 비율은 정답값이 아니라 도메인 특성에 따라 뒤집힐 수 있다는 점이 핵심이다.
...
- **정리는 각 테스트가 끝날 때 데이터를 명시적으로 비운다** — `@AfterEach`(JUnit 5라면 `AfterEachCallback` Extension)에서 전체 테이블을 `TRUNCATE`한다.

필자는 **트랜잭션 롤백에 의존하지 않는다** (`@Transactional`을 테스트에 붙여 끝나면 자동 롤백시키는 방식). 롤백은 빠르고 편하지만, **실제 커밋 이후에만 도는 로직**(DB 트리거, `@TransactionalEventListener(AFTER_COMMIT)`, 커밋 후 캐시 갱신 등)을 통째로 검증에서 배제한다. 롤백된 테스트는 "초록불인데 운영에선 다르게 도는" 위험을 남긴다. 그래서 매 테스트 후 모든 테이블을 비워 다음 테스트가 깨끗한 상태에서 시작하게 한다.

전체 `TRUNCATE`의 실전 함정 하나: 외래키 제약이 걸린 테이블은 순서 때문에 `TRUNCATE`가 막힌다. 정리 직전에 제약을 잠시 끄고(H2는 `SET REFERENTIAL_INTEGRITY FALSE`, MySQL은 `SET FOREIGN_KEY_CHECKS = 0`) 전부 비운 뒤 복원하면 삭제 순서를 신경 쓸 필요가 없다.

## 7. Flaky Test 원인 분류와 대응

**flaky test란, 코드를 바꾸지 않았는데 어떤 때는 통과(green)하고 어떤 때는 실패(red)하는 테스트**를 말한다. 같은 커밋을 CI에서 다시 돌리면 결과가 달라진다. 원인이 코드의 진짜 버그가 아니라 타이밍·실행 순서·공유 상태 같은 *테스트 환경의 비결정성*에 있어서, "가끔 빨간 CI"의 주범이 된다. 신뢰를 잃은 테스트는 팀이 실패를 무시하기 시작하는 순간 존재 이유를 잃는다.

테스트를 flaky로부터 지키려면 먼저 원인을 범주화해야 한다.
...
| 전략 | 장점 | 단점 |
|------|------|------|
| 테스트 후 `TRUNCATE` (`@AfterEach`) | 실제 커밋 경로를 그대로 검증, 모든 시나리오 동작 | 롤백보다 느림 |
| `@Transactional` + 롤백 | 빠름, 자동 정리 | 커밋·비동기·배치 시나리오를 검증에서 숨김 |
| DB 스냅샷 복원 | 가장 현실적 | 환경 세팅 부담 |

필자의 기본은 **테스트 후 `TRUNCATE`**다. 롤백은 편하지만 커밋 이후에만 도는 로직을 검증에서 빼버려, "테스트는 초록불인데 운영은 다르게 도는" 함정을 만든다. 속도 손해는 통합 테스트를 JVM 단위로 재사용하고 병렬화해서 상쇄한다.

## 9. JMH 마이크로벤치마크 — 측정 기반 최적화의 도구
...
JMH의 가치는 **"바꾼 코드가 정말 더 빨라졌는가"를 추측이 아니라 수치로 답하게 한다**는 점이다. 흔히 효과가 큰 변경 축은 (1) 자료구조 교체(선형 탐색 → 사전계산 인덱스/비트마스크), (2) 정밀도 표현 교체(`BigDecimal` → `long` cents 단위), (3) 스트림 → 인덱스 루프처럼 알로케이션을 줄이는 변경이다. **측정 없이는 어떤 최적화도 거짓**이다.
...
**Red-Green-Refactor**에서 시니어가 자주 건너뛰는 실수는 Refactor 단계다. 초록불이 들어오면 바로 다음 Red로 가지 말고, **중복과 네이밍을 즉시 정리**한다. Green 직후 리팩터링 없이 다음 테스트로 넘어간 코드는 결국 테스트 코드 자체가 유지보수 부채가 된다.
...
**1) 레이어 결정**
"저는 먼저 도메인 특성에 따라 피라미드 비율을 조정합니다. 상태 기반 의존성이 많고 리그레션이 치명적인 도메인은 통합 비중을 과감히 올립니다. 슬롯 서비스에서는 테스트 파일 447개를 통합 75% / 단위 25% / slice 0% 비율로 운영했습니다. Mock 관리 비용이 커서 통합 위주로 갔고, 직접 책임지는 어댑터가 적어 slice는 통합으로 흡수했습니다. 반대로 CRUD 성격 모듈이라면 slice 위주로 갔을 겁니다."
...
**4) 문화**
"테스트를 잘 짜는 것만큼 팀이 테스트를 쉽게 짤 수 있게 만드는 것도 중요합니다. 슬롯마다 반복되던 셋업 코드를 추상 클래스와 JUnit5 Extension으로 뽑아내, 새 슬롯은 구현체 하나와 추상 메서드 두 개만 구현하면 통합 테스트가 붙도록 인프라를 깔았습니다. 테스트 작성 비용을 낮춰야 팀이 실제로 테스트를 씁니다. flaky가 생기면 원인을 5가지(타이밍/순서/공유상태/외부/비결정)로 분류해 즉시 티켓을 끊고, 성능 작업은 JUnit이 아니라 JMH에서 검증합니다."
...
- [ ] 테스트 후 데이터 정리가 명시적(`@AfterEach` TRUNCATE 등)이며, 커밋 이후 로직까지 검증되는가
...
- [ ] 팀이 테스트를 쉽게 붙이도록 공통 셋업이 추상 클래스·Extension으로 인프라화돼 있는가
