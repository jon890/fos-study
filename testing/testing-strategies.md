...
필자는 실무에서 슬롯 게임 엔진에 **447개의 테스트**를 유지하고, 팀 차원 테스트 관습을 코드화하기 위해 **Cursor Rules 20종**을 운영해 "PR마다 어떤 테스트가 들어가야 하는가"를 규칙 레벨에서 고정한 경험이 있다. 이 글은 그 경험에서 나온 결정들을 구조화한 것이다.
...
**4) 문화**
"팀 차원에서는 Cursor Rules 20종으로 'PR마다 어떤 테스트가 필요한지'를 규칙화해 리뷰 마찰을 없앴습니다. flaky가 생기면 원인을 5가지(타이밍/순서/공유상태/외부/비결정)로 분류해 즉시 티켓을 끊고, 성능 작업은 JUnit이 아니라 JMH에서 검증합니다."
