...
> "재시도 큐 도입 후 평균 지연이 42ms 늘었다. SLA 가 300ms 라 여유가 있었고,
> 일시 오류로 인한 실패율이 1.8%에서 0.1%로 떨어지는 편이 이득이라 판단했다."

**수치 자리를 `0.xx%` 처럼 비워 둔 채 예시로 쓰지 않는다.**
이 문서의 예시를 그대로 옮겨 쓰는 흐름이 실제로 있었고,
`0.xx%` 가 채워지지 않은 채 다섯 곳에 퍼져 게시됐다.

- 채우지 않은 자리표시자는 없느니만 못하다
- 관리되지 않는 문서라는 인상을 주고, AdSense 심사에서는 "미완성 사이트" 판정 근거가 된다
- 실제 측정값이 없으면 수치 주장 자체를 빼고 판단만 남긴다
