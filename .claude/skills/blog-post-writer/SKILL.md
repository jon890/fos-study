...
2. **공개 가능 수위(L2) 준수** — 회사명/팀명은 OK, 비즈니스 도메인 고유 클래스명과 사업 의사결정은 X (세부 기준은 3번 항목 참조)
...
`## 내 기여 범위` / `## 협업 흐름` / `## 지금 보면` 3종 섹션 세트를 기계적으로 박으면 **이력서 bullet point** 같은 톤이 된다. 6번 항목에서 상세히 다뤘지만 다시 강조:
...
2. 의심되면 `WebSearch`로 짧게 검증 — `"<용어> spring boot 블로그"` 정도면 충분
...
본 규칙은 **fos-blog 등 자동 번호 매김 시스템에 sync 되는 문서에만** 적용된다. skill / docs / ADR / 코드베이스 내부 markdown 같이 자동 번호 매김 시스템 밖의 문서에는 적용하지 않는다 — 오히려 AI agent 가 본문 cross-ref ("1단계 진행", "2단계 선택") 할 때 명시적 숫자가 식별을 쉽게 한다.
...
3. **얕은 주제 배제 판단** (7번 항목) — 커밋 수 5\~6개 이하 + 클래식한 결론 + 재사용 인사이트 부족 시 배제/후순위 후 사용자 동의
...
9. **글 자가 점검** —
    ...
    - `grep -n "§" <file>` (잔존 시 평문으로 치환, 14번 항목 H)
    - **영문 용어 직역 점검** (14번 항목 I) — 글을 다시 읽으며 영문 그대로 쓴 용어 중 한글이 더 익숙해 보이는 게 있으면 `WebSearch`로 짧게 확인 후 한글 우선 + 영문 병기 패턴 적용. 모든 단어를 검색하지 말고 의심 가는 것만.
    - `grep -nE "^#+ [0-9]+\." <file>` (heading 숫자 prefix 0개 확인, 14번 항목 K) + 본문 숫자 cross-ref 잔존 점검 (`grep -nE "(위 [0-9]+|[0-9]+개 항목|[0-9]+번 항목|섹션 [0-9]+)"`)
