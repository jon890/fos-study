...
**정의** — 링크 자체는 깨지지 않았지만 **시각적으로 흩어져 있어** 독자가 사례/링크 관계를 빠르게 파악하기 어려운 패턴.

이 축은 fos-study CLAUDE.md 의 "가시성 원칙" 과 blog-post-writer 스킬 14-J 항목과 동일한 기준을 적용한다.

**검사 항목**

1. **콤마-줄글 사례 단락** — 정보 항목 4개 이상이 콤마/`·`/`+` 로 한 줄에 이어진 paragraph
2. **긴 인라인 부연 패턴** — `[제목 — 긴 부연](path) — 또 줄글` 형태. `](path) — ` 뒤에 70자 이상이 같은 줄에 이어짐
3. **누적성 섹션의 평탄화 부재** — `## 실제 적용 사례` / `## 관련 사례` / `## 적용 케이스` 같은 헤더 아래에 bullet 1개만 있고 그 bullet 이 1, 2번 패턴과 겹침 → "사례 누적 시 확장 어려움" 으로 플래그
4. **인라인 링크 폭주 단락** — 한 paragraph 안에 markdown 링크가 5개 이상 — 본문 흐름이 끊김

**전처리 — 코드/표 마스킹 (false positive 차단)**

휴리스틱이 가시성 위반으로 잘못 잡는 케이스가 많다. 검사 전에 다음을 마스킹한다.

- **펜스 코드 블록**: `\`\`\`...\`\`\`` 사이는 검사 대상에서 완전 제외 (Java/JS 의 문자열 concat `"a" + b + "c"` 가 잘못 잡히는 주된 원인)
- **인라인 코드**: 백틱 1개로 감싼 부분도 제외 (예: `` `2 + 3` ``)
- **표 행**: `|` 로 구분된 표 행은 콤마 카운트에서 별도 임계치 (콤마는 표 데이터 구분용으로 자연스러움)

**시그널 우선순위 (강 → 약)**

1. **`,` 콤마 또는 `·` 4회 이상** — 가장 강한 시그널. 자연어 문장에서 4개 이상 항목 enumeration 은 거의 항상 줄글이 약함
2. **` + ` 4회 이상** — 중간 시그널. `+` 는 문자열 concat / 합산식 / 기술 스택 나열 등 줄글 외 용도가 많아 단독으로는 약함. 코드/표 마스킹 후에만 적용
3. **` + ` 3회** — 가장 약함. **단독으로는 보고하지 않는 게 좋음**. 다른 신호와 결합될 때만 표시

**의도적 줄글 패턴 — 보고 제외**

다음 paragraph 는 콤마/`+` 가 많아도 가시성 위반이 아니라 **의도된 줄글**이다. 가능하면 사전 필터링한다.

- **면접 답변 카드의 키워드 enumeration**: 한 호흡에 핵심 키워드를 나열하는 답변 발화 (`interview/**` 디렉터리는 해당 컨텍스트가 강함)
- **자기소개 / 1분 발화 템플릿**: 한 문장으로 압축해야 하는 발화
- **기술 스택 단순 나열**: `Prometheus + Grafana + Tempo + Loki + OTel Collector` 처럼 5개 이내 도구 이름만 콤마/`+` 로 잇는 경우 (각 항목이 짧음)
- **메모리/사이즈/시간 합산식**: `7 + 100 + 8 = 115 bytes` 같은 계산식
...
# 1. 인라인 코드 마스킹
text = re.sub(r'`[^`]+`', '', text)
# 2. 줄 단위로 검사하되, 표 행(| 시작) 과 면접 디렉터리는 별도 처리
# 3. 의도적 줄글 패턴은 keyword 길이 / 디렉터리 컨텍스트로 필터
```

**자동 수정 금지** — 가시성은 글 흐름과 의도가 섞여 있어 일괄 변환이 위험하다. cross-link 와 마찬가지로 후보만 제시하고 수정안은 사용자가 선택.
...
| 파일 | 줄 | 패턴 | 제안 |
|---|---|---|---|
| devops/graceful-shutdown.md | 200 | 콤마-줄글 4개 항목 | 컴포넌트별 bullet 분리 |
| architecture/foo.md | 12 | 긴 인라인 부연 (90자) | 제목 짧게 + 다음 단락에 부연 |
```
