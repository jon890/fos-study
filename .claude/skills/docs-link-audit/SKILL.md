...
1. **콤마-줄글 사례 단락** — 정보 항목 4개 이상이 콤마/`+`로 한 줄에 이어진 paragraph
   - 정규식 휴리스틱: 한 줄에 ` + ` 가 3회 이상 등장하거나, 콤마(`,` `·`)가 4회 이상 등장하면서 그 줄이 markdown 링크/리스트 항목 본문인 경우
2. **긴 인라인 부연 패턴** — `[제목 — 긴 부연](path) — 또 줄글` 형태
   - 정규식 휴리스틱: `[...](...)` 안의 텍스트가 50자 이상이거나, `](path) — ` 뒤에 70자 이상의 줄글이 같은 줄에 이어지는 경우
3. **누적성 섹션의 평탄화 부재** — `## 실제 적용 사례` / `## 관련 사례` / `## 적용 케이스` 같은 헤더 아래에 bullet 1개만 있고 그 bullet이 위 1, 2번 패턴과 겹치는 경우 → "사례 누적 시 확장 어려움" 으로 플래그
...
# 3. 누적성 섹션 휴리스틱 — 헤더 텍스트가 "사례", "케이스", "적용" 으로 끝나고
#    바로 아래에 H3 / 표 없이 bullet 1개만 있는 경우
```
