...
| 2026.04–현재 | OCR 비즈니스 오류 관측: HTTP 200 응답 안의 오류 코드 지표와 주간 원인 분류 | [ocr-business-error-monitoring.md](ocr-business-error-monitoring.md) |
...
- **AI 웹툰 제작 도구 MVP**: 웹소설 → 세계관/캐릭터/각색/글콘티 → 60컷 이미지 6단계 파이프라인
  - `/planning` → `/plan-and-build` → `/build-with-teams`로 계획·구현·검토를 분리하고, 디자이너 목업 통합 절차를 `/integrate-ux`로 정리
  - Gemini 모델 전략(품질 우선, 429 대체 경로, 전역 요청 제한 추적), 통합 분석으로 토큰 75% 절감
  - 글콘티 환각 차단: Grounding 블록, Continuation 재주입, Project 단위 Context Cache 적용
  - 캐릭터 외형 고정: mode 분기와 기본 시트 이미지 자동 첨부
  - 디자이너 협업: Container/Presenter, Layout Primitives, 파일 소유권 표로 충돌 감소
  - 타입 시스템: Zod와 Prisma를 레이어별로 분리하고 mapper로 변환 경계 관리
