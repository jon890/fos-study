...
- 지원 형식별 회귀 표본 보강과 오류 분류 자동화 변경은 검토 브랜치에서 확인 중
- 읽기 순서, 스캔본 하단 각주, 참고문헌 절단처럼 정답지 재검수에서 분리한 품질 결함은 개별 개선 대상으로 관리
- 검증 기준점과 운영 배포 결과를 계속 재채점해 정답지·파서·평가 코드 중 어느 쪽이 달라졌는지 분리
