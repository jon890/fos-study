...
Document Parser의 처리 속도를 여섯 차례에 걸쳐 손봤다.
[Document Parser 운영과 개선](./document-parser.md)에서 전체 맥락을 다룬다.
OCR 호출 구조를 병렬로 바꾸고, 사용하지 않는 렌더링을 제거하고, PPTX 이미지 처리 방식을 다시 설계했다.
...
이 실측은 운영 지표와 단계별 처리 시간 로그를 함께 확인했기 때문에 가능했다.
