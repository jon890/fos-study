...
운영에서는 이 누수를 막기 위해 `MAX_TASKS_PER_WORKER=3`을 적용하고 있었다. 워커가 작업 세 건을 처리하면 종료하고 새 프로세스를 시작한다. 운영체제가 종료된 프로세스의 메모리를 회수하므로 RSS는 낮아지지만, 새 워커가 모델 가중치를 읽고 cache를 초기화하는 비용이 매번 발생한다.
...
glibc 매뉴얼의 `man 3 malloc_trim` 설명에 따르면 `malloc_trim()`은 `sbrk(2)`나 `madvise(2)`를 호출해 힙의 여유 메모리를 반환하려고 시도한다.
...
Adam Johnson(Django 코어 컨트리뷰터)의 글은 워커 재활용을 단순하고 예측 가능한 대응으로 설명한다. 우리는 워커 재활용(`MAX_TASKS=3`)과 `malloc_trim`을 함께 적용했지만, 두 방법의 효과를 분리해 측정하지는 못했다.
