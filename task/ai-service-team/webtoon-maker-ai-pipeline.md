...
사내 웹툰 제작 조직이 시험 삼아 만든 도구에서 쓸 만한 결과가 나오는 것을 확인하고, 실제 제작에 쓸 제품으로 만들어 달라고 요청했다.
개념 증명은 이미 있었으므로 과제는 "되는가"가 아니라 제품화였다. 운영자가 단계마다 결과를 검토하고 수정하며 작업 상태를 보존하는 구조가 필요했다.

지금은 웹툰 제작 속도를 높이는 내부 도구로 사용하고 있다.
만든 결과물을 인스타그램과 유튜브 쇼츠에 올려 반응을 확인했고, 정식 서비스로 출시한 것은 아니다.
