...
주간 오류 분류를 반복한 뒤 오류 로그 비율이 얼마나 내려갔는지는 아직 이 문서에 적혀 있지 않다.
아래 표의 `기록` 칸을 채워야 이 수치를 다른 글에서 인용할 수 있다.

| 항목 | 단위 | 집계 조건 | 기록 |
| --- | --- | --- | --- |
| 비교한 기간 | 주 | 시작 주와 끝 주의 날짜를 함께 적는다 | 아직 적지 않음 |
| 비교한 호출량 | 주 평균 건수 | 어느 서비스의 어떤 호출을 셌는지 적는다 | 아직 적지 않음 |
| 개선 전 오류 로그 비율 | % | 분모와 분자에 각각 무엇을 넣었는지 적는다 | 아직 적지 않음 |
| 개선 후 오류 로그 비율 | % | 개선 전과 같은 분모를 썼는지 적는다 | 아직 적지 않음 |
| 실패 요청만 세도록 바꾼 기준 | 문장 | 어떤 로그를 집계에서 빼고 어떤 로그를 남겼는지 적는다 | 아직 적지 않음 |

분모와 분자를 함께 적는 이유는, 같은 백분율이라도 전체 요청 대비인지 오류 로그 대비인지에 따라 뜻이 달라지기 때문이다.
