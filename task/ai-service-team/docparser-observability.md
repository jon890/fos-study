...
## metrics·로그 관측성 개선: 고아 db 전수 검사과 로그 정리
...
- **고아 db 전수 검사**: `worker_death_monitor` 루프가 매 주기 `PROMETHEUS_MULTIPROC_DIR` 안의 `*_<pid>.db` 파일에서 PID를 뽑아 `psutil.pid_exists(pid)`로 생존 여부를 확인하고, 죽은 PID면 `mark_process_dead`로 정리한다.
