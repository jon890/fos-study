...
derived 합성은 치환 순서를 의존 그래프로 표현한다. 새 치환 함수는 그래프 끝에 추가할 수 있어 갱신 순서를 별도 명령형 코드로 관리하지 않았다.
...
**외부 라이브러리와의 키 충돌**. 특정 키 네임스페이스가 라이브러리 내부 예약어와 겹쳐 잘못된 번역이 표시됐다. 앱 키에 prefix를 적용해 외부 라이브러리의 번역 키 공간과 분리했다.
...
Svelte 5의 rune(`$derived`)을 쓰면 파생 상태를 더 직접 표현할 수 있다. 더 중요한 회고는 다른 지점에 있다.
