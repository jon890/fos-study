...
전체 구조를 한 번에 바꾸기 어려워 작은 영역부터 1년 반에 걸쳐 정리했다. 작은 PR을 누적하는 과정에서 먼저 적용한 구조의 한계가 드러나기도 했고, 아직 해결하지 못한 문제도 남아 있다.
...
static 호출, 강결합과 거대한 파라미터 객체가 함께 얽혀 테스트 대상을 분리하기 어려웠다. 첫 슬롯 작업에서 이 제약을 확인한 뒤 전체 구조를 한 번에 바꾸기보다 테스트 가능한 경계부터 나눴다.
...
템플릿화로 흐름을 정리하고 나서 본격적으로 static을 제거하기 시작했다. 한 번에는 불가능했다. PR마다 하나의 영역씩 풀었다.
...
static 호출을 하나씩 주입 방식으로 바꾸자 다음 의존 관계가 드러났다. 빈 전환 뒤에는 갱신 중 읽기 문제도 나타나 락을 추가해야 했다. 각 PR에서 한 경계와 그에 따라 드러난 문제를 처리하는 방식으로 범위를 관리했다.
...
이 영역은 큰 PR 하나로 끝내기 어려웠다. 팀 일정 안에서 코드를 수정할 때마다 주변 구조도 함께 개선하는 방식으로 진행했다. 1년에 걸쳐 누적한 작은 PR을 통해 static 호출을 빈 주입으로 바꾸고 통합 테스트 기반을 만들었다.
