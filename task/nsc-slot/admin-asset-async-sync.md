...
전략을 분리하자 심볼 파일명 규칙이 바뀌어도 해당 구현체만 수정하면 됐다. 서비스 레이어는 같은 인터페이스를 사용하므로 다른 에셋 유형에는 변경이 번지지 않았다.

### 전용 ThreadPool로 도메인 영향 격리
...
### `@ConditionalOnProperty`로 환경별 Bean 활성화

슬롯메이커 FTP 연동은 운영/스테이지 환경에서만 동작해야 한다. 개발 환경이나 로컬에서 빈이 생성되면 FTP 연결 실패로 앱이 뜨지 않는다. 관련 Bean 전체에 `@ConditionalOnProperty(name = "nsc.slotmaker.ftp.enabled", havingValue = "true")`를 적용했다.
...
운영에 영향을 주는 기능은 설정이 없을 때 비활성화되도록 했다. 다른 환경에 설정이 누락되더라도 외부 FTP에 잘못 연결하기보다 기능이 시작되지 않는 쪽을 선택했다.
