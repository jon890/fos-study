...
```mermaid
flowchart TD
    A[유저 스핀 요청] --> B[RccHandler]
    B --> C["[튜토리얼/치트 스핀] → 패스 (RCC 발동 안 함)"]
    B --> D["[시뮬레이터 실행 중] → 패스"]
    B --> E["[일반 스핀, 캐시 있음] → 캐시된 결과 반환"]
    E --> F["비동기: 다음 캐시 생성 트리거"]
    B --> G["[일반 스핀, 캐시 없음] → 일반 스핀 결과 반환"]
    G --> H["비동기: 캐시 생성 트리거"]

    subgraph BG["RccCacheGenerator (백그라운드)"]
        I["RccSpinResultAnalyzer → 어떤 결과를 캐시할지 판단"]
        J["스핀 시뮬레이션 → 조건 충족 결과만 DB 저장"]
        I --> J
    end
