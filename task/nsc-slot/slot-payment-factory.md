...
- **제네릭 체커, Map Factory**는 슬롯 타입이 늘어날 때 조건 분기를 줄여준다. 실제로 초기에는 `if-else`였고, Way 타입을 추가할 때 리팩터링했다
...
- Spring의 **`List<Interface>` 자동 수집**을 이용하면 구현체를 생성자로 주입할 수 있다. `ApplicationContext.getBeansOfType()`을 직접 호출하지 않아 애플리케이션 코드가 컨테이너 조회에 의존하지 않는다
