...
AGENTS.md / blog-post-writer 룰 중 4가지를 본다.
...
|---|---|---|
| 3. Orphan | `orphan-doc-auditor` | `.Codex/agents/orphan-doc-auditor.md` |
| 4. Cross-link | `cross-link-auditor` | `.Codex/agents/cross-link-auditor.md` |
| 7. README 정합성 | `readme-integrity-auditor` | `.Codex/agents/readme-integrity-auditor.md` |
...
2. **Codex maintainer / writer가 큰 정리를 한 직후** — 반드시 diff-validation 모드로 본다 (Q6 축 활성). git diff 기준 *실제 개선인지 문장만 옮긴 건지* 판단
...
- **bold-paren 자동 분리는 nested 괄호·메서드명·코드 인용을 깬다**
  ...
  - **메서드명 bold** — `**doOpen()**`, `**update()**`, `**canRetry()**` 같은 메서드 시그니처는 AGENTS.md 룰상 코드 인라인 (`` `doOpen()` ``) 으로 바꿔야 한다. `**doOpen**()` 로 분리하면 더 어색.
    - 사전 필터: 괄호 안이 비어 있거나 `()` 패턴이면 skip. 별도 round 에서 backtick 화 제안만.
  - **예외 파일** — `AGENTS.md`, `*Codex-usage-reflection*.md`, `*blog-post-writer*.md` 같이 룰 자체를 인용·예시로 보여주는 문서는 의도적 위반 형태가 정답.
...
`scripts/docs_score.py` — 위반을 가중 감점으로 합산한다. 위반 0이면 score 0(만점).

| 채점 축 | 가중 | 자동 채점 근거 |
|---|---|---|
| broken_link | 5 | 링크 대상 파일 존재 여부 |
| readme_missing | 3 | 폴더 README 에 같은 폴더 .md 등재 여부 |
| orphan_doc | 2 | 어느 문서에서도 링크 안 됨 (진입점 제외) |
| style_tilde / section / bold_paren / italic_paren | 2 | 문체 정규식 (코드 마스킹 후) |

가시성(축 5)·cross-link(축 4) 은 주관 판단이라 점수에서 **제외**한다.

### 사용

```bash
python3 .Codex/skills/docs-audit/scripts/docs_score.py          # 측정 + 직전 대비 delta
python3 .Codex/skills/docs-audit/scripts/docs_score.py --save   # 게이트 통과 시 history 기록
python3 .Codex/skills/docs-audit/scripts/docs_score.py --json    # 기계 판독용
```
...
점수 함수가 틀리면 잘못된 방향으로 개선하게 된다.
정규식 detector 는 false positive 를 반드시 spot-check 한다.

- `style_bold_paren` 은 bold 안 markdown 링크(`[..](..)`)·통괄호 강조(`**(VO)**`)·메서드명을 제외해야 한다.
- `~` 는 shell home path(`~/...`)·escape(`\~`)·코드 블록을 카운트에서 빼야 한다.
- detector 를 고칠 때마다 잔존 건을 직접 grep 으로 확인한 뒤 가중치를 신뢰한다.
...
- 저장소 컨벤션: `/Users/nhn/personal/fos-study/AGENTS.md` "하위 문서 링크" / "가시성 원칙" / "마크다운 Bold + 괄호 패턴"
- 문체 룰 출처: `.Codex/skills/blog-post-writer/SKILL.md` 14-G/H/I/J 항목
