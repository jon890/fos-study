...
인사이트 독창성·AI 티·서사 흐름·회고 자연스러움 같은 **의미 품질은 `scripts/blog_judge.py` 가 Codex CLI 로 채점**한다.
...
- **자동 채점** — `scripts/autoscore_hook.sh` 를 PostToolUse(Write|Edit) hook 으로 등록(`.Codex/settings.local.json`).
