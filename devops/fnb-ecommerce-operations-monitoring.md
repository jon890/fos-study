...
# F&B / e-Commerce 운영 장애 대응과 모니터링 — 백엔드 관점 정리
...
실무 관점에서도 마찬가지다. 여러 외식 브랜드를 운영하는 다채널 F&B 조직은 "안정 운영"을 가장 앞에 둔다. 코드 스타일이나 알고리즘보다 "피크타임에 503 어떻게 줄였나", "PG 장애 때 어떻게 대응했나", "재고 동기화 깨졌을 때 어떻게 회복했나" 같은 운영형 문제가 시니어 백엔드 역량의 핵심이 된다. 이 글은 그 핵심을 정면으로 다룬다.
...
traceId 재전송 경험은 정확히 이 지점에 닿는다. "동기 호출에서만 traceId가 살아 있고 Kafka로 넘어가는 순간 끊겼다 → 메시지 헤더로 명시적으로 실어 보내 비동기 구간까지 같은 traceId로 추적 가능하게 만들었다" 같은 서술이 강한 운영 신호가 된다.
...
"graceful shutdown 503 대응" 경험은 이 루프의 1\~2번에 닿는다. 배포 중 503 비율이 튀었을 때 "직전 변경이 원인" → "graceful shutdown으로 in-flight 요청 종료 후 종료" → "503 비율 회복" 흐름은 첫 5분 루프를 정확히 따른 좋은 케이스다.
...
좋은 포스트모템의 가장 큰 특징은 "사람을 탓하지 않는다"는 점이다. "A가 실수했다"가 아니라 "그 실수가 운영 단계까지 도달할 수 있는 구조였다"로 쓴다. 같은 톤이 시니어 운영 역량의 신호로 읽힌다.
...
운영 장애를 설명할 때는 **STAR + 숫자 + 트레이드오프**로 정리한다.

- **Situation**: "점심 피크타임에 5xx 비율이 평소 0.1%에서 2%까지 튀었다."
- **Task**: "사용자 영향이 직접 보이는 상황이라 원인 파악보다 완화가 우선이었다."
- **Action**: "직전 배포가 의심돼 graceful shutdown 흐름을 점검했고, in-flight 요청이 강제 종료되며 503으로 떨어지는 패턴을 확인했다. 종료 hook에서 헬스체크를 먼저 unready로 바꾸고 in-flight 요청이 종료된 후에 컨테이너를 내리도록 수정했다."
- **Result**: "이후 배포 시 503 비율이 평시 수준으로 돌아왔고, 같은 패턴의 재발을 막기 위해 배포 파이프라인에 graceful shutdown 점검 단계를 추가했다."
- **Trade-off**: "graceful shutdown 시간이 길어지면 배포 속도가 느려져서, 최대 대기 시간을 60초로 캡을 걸어 둔 점도 같이 설명한다."

운영 깊이를 묻는 주제 — "캐시 정합성 어떻게 다뤘나", "traceId 비동기 구간 어떻게 살렸나", "PG 장애 때 어떻게 대응했나" — 에는 같은 프레임을 그대로 쓰면 된다.

## 자주 나오는 운영 질문과 정리 포인트

**Q. 피크타임에 가장 먼저 무너지는 자원은 무엇인가**
...
**Q. 캐시 정합성은 어떻게 보장하는가**
A. "절대 안 깨지게"는 비현실적이고, "깨져도 N분 안에 수렴한다"를 목표로 한다. TTL을 짧게 잡고, 변경 이벤트로 명시적 invalidate, 최후 보루로 강제 리프레시 잡을 둔다. 실제로 다룬 정합성 이슈 사례와 어떤 레버로 수렴 시간을 줄였는지 같이 정리한다.

**Q. 운영 중 traceId가 끊기면 어떻게 하는가**
A. 끊긴 구간을 먼저 찾는다. 대개 비동기 구간(Kafka, 스레드 풀, 배치) 진입 시점이다. 메시지 헤더 또는 컨텍스트 전파로 traceId를 명시적으로 실어 보내는 처리로 복구한다. 직접 수정해본 경험을 그대로 풀어내면 강하다.

**Q. graceful shutdown은 왜 중요하고 어떻게 구현하는가**
