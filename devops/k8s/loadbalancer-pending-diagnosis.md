...
trust 인증과 내부 프록시는 관리형 쿠버네티스의 control plane 영역이라 사용자가 직접 고칠 수 없다. 내 쪽에서 할 수 있는 조치(서브넷 명시)는 끝났고, 나머지는 증거를 정리해 클라우드 지원 채널에 넘기는 게 맞다고 판단했다. 이 글을 쓰는 시점 기준으로 문의를 준비 중이고, 장애가 해소되면 LB 발급과 IP 고정으로 이어질 예정이다.
