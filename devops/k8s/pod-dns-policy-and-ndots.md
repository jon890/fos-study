...
호출 주소가 힌트였다. 내부 서비스 이름이 아니라 **공인 도메인**이다. 관리자 서버 → 공인 LoadBalancer → Ingress Controller → API 서버 순으로 한 바퀴 돌아 들어온다. 그 과정에서 출발지 IP가 SNAT으로 바뀌고, 전달 헤더도 프록시가 다시 쓴다. API 서버 입장에서는 밖에서 온 요청과 구분할 근거가 사라진다.
