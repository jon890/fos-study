...
- graceful shutdown 503 해결 같은 사례 카드를 첫 5분 흐름과 어떻게 묶어 설명할지 정리한다.

대상 독자는 F&B/커머스 도메인에서 SRE 협업이 필요한 시니어 백엔드다. 관련 인접 문서가 이미 있다면(예: [task/ai-service-team/graceful-shutdown-503-fix.md](../task/ai-service-team/graceful-shutdown-503-fix.md)) 사례 본문은 그쪽에 두고 이 문서는 첫 5분 운영 플레이북 + 관측성 기본기 hub 역할로 좁힌다.
...
graceful shutdown 503 해결 경험은 첫 5분 플레이북과 직접 연결된다. 사례를 다음 흐름으로 재구성하면 설명이 자연스럽다.
...
이 사례가 강한 이유는 "장애를 풀었다"가 아니라 "관측성·플레이북 자체를 영구적으로 개선했다"까지 들어가기 때문이다.

## 로컬 실습 환경: docker-compose로 첫 5분 시뮬레이션

빠르게 손에 익히려면 다음 4개 컨테이너로 충분하다.
...
"장애 대응 경험"을 설명할 때는 다음 4단 구조가 안전하다.

1. **표면**: 어떤 비즈니스 지표가 어떻게 움직였는지부터 말한다. ("분당 결제 실패율이 0.3%에서 4%로 튀었다.")
2. **좁히기**: golden signal로 영역을 어떻게 좁혔는지. ("우리 서버 latency와 saturation은 정상이었고, PG 호출 segment의 p99만 튀고 있었다.")
3. **결정**: 격리·롤백·확장 중 무엇을 왜 골랐는지. ("외부 의존성 의심이 강해 PG 우회 경로 쪽 트래픽 비중을 30%로 올렸다.")
4. **영구 개선**: 알람·플레이북·코드 중 무엇을 바꿨는지. ("이후 PG별 호출 latency·실패율 패널과 알람을 분리했고, circuit breaker 임계치를 시간대별로 다르게 잡도록 했다.")

graceful shutdown 503 사례는 위 4단에 정확히 매핑된다. "장애를 빨리 풀었다"보다 **"같은 종류의 장애가 다시 일어나지 않도록 관측성과 배포 sequence를 영구적으로 바꿨다"**를 강조한다.

다음과 같은 후속 논의도 미리 준비한다.

- "그 알람이 너무 자주 울렸다면 어떻게 튜닝할 것인가" → 시간대 baseline + multi-window multi-burn-rate.
- "트래픽이 적은 시간대에는 골든 시그널이 의미가 있는가" → 분모가 작을 때는 비율 알람 대신 절대 건수 알람으로 바꾼다.
...
알림 기준은 “5분 이동 실패율이 평소 동일 요일/시간대 대비 2배 이상 + 절대 실패 건수 N건 이상”처럼 비율과 건수를 함께 둔다. 장애 종료 후 postmortem에는 감지 시각, 사용자 영향 범위, 매출/주문 영향, 원인, 완화 조치, 재발 방지 액션, 대시보드/알림 보강 항목을 반드시 남긴다. 핵심은 “롤백 여부를 직전 배포 감이 아니라 비즈니스 지표와 dependency split으로 결정한다”는 원칙이다.
