...
# Java 라이브 코딩: HashMap과 문자열 패턴 집중 훈련
...
첫째, 면접관 입장에서 평가 포인트가 풍부하다. 한 문제 안에서 다음을 모두 관찰할 수 있다.
...
둘째, 백엔드 실무와 연결된다. 요청 로그에서 패턴을 추출하거나, 중복 키를 탐지하거나, 특정 상태를 카운팅하는 작업은 실제 서비스 운영에서 매일 마주친다. 셋째, 30~45분이라는 제한된 시간 안에 "생각 → 코드 → 검증"을 모두 보여줄 수 있을 만큼 범위가 적절하다.
