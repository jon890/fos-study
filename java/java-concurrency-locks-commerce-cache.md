...
외식 커머스 환경에서는 점심 직전과 저녁 직전 트래픽 피크 직전에 운영자가 메뉴 가격, 품절 여부, 할인율을 바꾸는 패턴이 흔하다. "변경은 분당 수 건, 조회는 초당 수천 건"이라는 비대칭이 핵심이다. 이때 단순 `synchronized`로 막아 버리면 운영자 1명의 메뉴 갱신이 점심 피크의 모든 조회를 줄 세우는 사고가 난다. 동시성 락은 `synchronized` vs `ReentrantLock` 비교에서 끝나기 쉬운데, 실제 차별점은 "왜 read-heavy 캐시에서는 RWLock이 모자라고 StampedLock의 optimistic read가 필요한가"를 설명할 수 있느냐다.
...
`volatile`은 단일 변수의 가시성만 보장하고 복합 연산의 원자성은 보장하지 않는다. 캐시 객체 전체를 통째로 갈아 끼우는 패턴에서는 `volatile` 하나로 충분할 수 있지만, "맵 안의 한 항목만 수정"에는 절대 부족하다. 이 구분이 가장 자주 헷갈리는 지점이다.
...
읽기 비중이 큰 정적 마스터 데이터 캐시는 어느 도메인에서나 같은 구조의 문제다. 정적 마스터 데이터(설정 테이블, 정책 값)는 운영자가 가끔 바꾸고, 처리 스레드는 매 요청마다 읽는다. 외식 커머스로 옮기면 다음으로 매핑된다.

| 정적 마스터 데이터 도메인 | 커머스 도메인 |
| --- | --- |
| 정적 설정 테이블 | 매장별 메뉴 마스터 |
| 정책/비율 테이블 | 프로모션/할인율 정책 |
| 운영자 콘솔의 정적 데이터 변경 | 점주/본사의 메뉴/가격/품절 변경 |
| 매 요청의 정책 적용 계산 | 매 주문의 가격 계산 |
...
"동시성을 어떻게 다루는가" 또는 "캐시 갱신 중 조회 일관성을 어떻게 보장하는가" 같은 문제는 다음 흐름으로 푼다.

1. **상황 정의로 시작한다.** 조회는 초당 수천, 변경은 분당 수 건 수준의 비대칭이라 read-heavy 정책 캐시 패턴으로 분류한다.
2. **선택지 비교를 짧게 깐다.** synchronized → 직렬화 비용, RWLock → read도 락 비용, StampedLock optimistic read → read 사실상 무비용, swap 패턴 → 부분 갱신 불가.
3. **선택과 근거를 댄다.** 데이터 양이 수천 건 수준이고 일관된 스냅샷 단위로 운영자가 적용하는 패턴이라 AtomicReference swap을 1차로 채택한다. 이후 부분 갱신 요건이 추가되면 StampedLock 기반으로 옮긴다.
4. **유사 구조와 잇는다.** 읽기 비중이 큰 정적 마스터 데이터 캐시도 같은 구조다. 운영자 콘솔 변경 시 새 스냅샷을 빌드해 통째 swap하는 방식으로 조회 latency 영향을 거의 없앨 수 있다. 외식 커머스 메뉴/프로모션 캐시도 같은 모델로 풀린다.
5. **trade-off를 먼저 인정한다.** StampedLock은 재진입이 안 되고 사용 규칙이 까다로워서 팀 코드 리뷰 시 패턴을 가이드 문서로 고정해야 한다.

이 흐름은 단순히 키워드를 나열하는 대신 read/write 비대칭을 보고 도구를 고른다는 점을 분명히 한다.
...
- 도구 이름만이 아니라 read/write 비율과 trade-off로 선택을 설명할 수 있는가.
