...
- 단일 문서 원천에서 사내 위키, 업무, 문서, 음성 STT와 공간 메타데이터를 포함하는 구조로 확장했습니다.
...
- Spring Boot 3·Java 17 기반 신규 슬롯 8종을 개발했습니다.
...
- 백그라운드 결과 캐시 시스템을 6종의 슬롯에 적용했습니다.
- 테스트가 어려운 레거시 코드를 1년 반 동안 작은 변경으로 통합 테스트 가능한 구조로 전환했습니다.

### 성능과 동시성 문제 해결

- 가중치 랜덤 선택을 O(n) 누적합에서 O(1) Alias Method로 변경했습니다.
- JMH에서 `ThreadLocalRandom` 70.241 ops/s와 `SecureRandom` 1.197 ops/s를 비교해 난수 생성기를 선택했습니다.
