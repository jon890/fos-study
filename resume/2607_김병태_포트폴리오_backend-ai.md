...
| 다중 소스 RAG 색인 | 느린 I/O와 중간 실패, 원천별 확장 | 배치 구조와 주요 처리 단계 개발 | Java 21, Spring Batch, OpenSearch |
| 문서 파싱 품질 플랫폼 | 변환 품질 회귀와 워커 운영 불안정 | 품질 검증·병렬화·운영 개선 | Python, FastAPI, OCR, golden set |
| OCR API·배포 안정화 | 오토스케일 연결 실패와 호출 경로 전환 | 호출자·모델·인프라 경계 개선 | Java 21, Reactor Netty, gRPC, Kubernetes |
...
Spring Batch 구조와 Reader, Processor, Writer, OpenSearch 색인의 주요 처리 단계를 개발했습니다.
여러 기여자가 함께 만든 서비스이므로 전체 파이프라인의 단독 소유로 표현하지 않습니다.
...
- 중복 CUDA 의존성을 제거해 이미지 압축 크기를 9.89GB에서 6.82GB로 줄였습니다.
...
모델 서버의 배포와 오토스케일 전환 때 준비되지 않은 인스턴스로 요청이 가거나 처리 중인 연결이 끊겼습니다.
API Gateway를 제거한 뒤에는 HTTPS, 경로 변환과 IP 접근 제어를 직접 책임져야 했습니다.

### 역할

- OCR API의 연결 풀과 재시도 정책 개선
- 모델 서버의 기동·종료 순서 개선
- 공인 LoadBalancer와 Ingress 기반 호출 경로 구성
- 계층 간 요청 식별자 전파와 검증 도구 작성

### 설계와 구현

- 준비 신호 뒤에 Envoy가 포트를 열도록 기동 순서를 고정했습니다.
- gRPC 처리 시간에 맞춰 종료 유예와 프로세스 신호 전달을 조정했습니다.
- 커넥션 최대 수명과 2초 연결 제한을 두고, 상대에 도달하지 않은 오류만 다시 보냈습니다.
- Gateway부터 모델 서버까지 같은 `X-Request-Id`를 전파했습니다.
- 접근 제어는 설정 존재가 아니라 허용되지 않은 주소가 실제로 차단되는지 검증했습니다.

### 결과

- 도달할 수 없는 모델 주소의 연결 실패 시간을 약 30초에서 2.1초로 줄였습니다.
- 기동과 종료 양쪽의 실패를 서버와 호출자 계층에서 함께 방어했습니다.
- 기존 경로와 새 경로의 응답 등가성과 지속 부하를 검증할 수 있게 했습니다.
...
제품 요구사항, 프론트엔드, 백엔드, 데이터베이스, AI 호출 구조와 에이전트 개발 하네스를 주도했습니다.
...
- 이어진 12일 동안 구조, 관측성, 오류 처리와 통합 테스트를 보강했습니다.
