...
- 단일 문서 원천에서 위키, 업무, 문서, 음성 STT와 공간 메타데이터로 확장했습니다.
...
- 직접 만든 클라우드 CLI로 운영과 같은 이미지의 테스트 인스턴스를 생성했습니다.
- 검증이 끝나면 인스턴스를 종료해 운영 영향과 비용을 줄였습니다.
...
- 카나리 검증 후 RSS가 일정 범위에서 유지되고 회귀가 없음을 확인했습니다.
...
- 가중치 랜덤 선택을 Alias Method로 바꿔 시간 복잡도를 O(n)에서 O(1)로 줄였습니다.
- JMH로 `ThreadLocalRandom`과 `SecureRandom`의 용도와 처리량을 비교했습니다.
