...
외식·매장 디지털 채널 운영사는 사내 어드민과 매장/가맹점용 웹은 여전히 JSP + jQuery + 서버 세션 기반인 경우가 많고, 동시에 자사 앱과 외부 파트너 연동은 토큰 기반 API로 굴린다. 세션과 토큰의 차이를 따질 때 핵심은 단순 정의가 아니라, **하나의 인증 인프라 위에서 레거시 채널과 신규 채널이 공존하는 상황을 어떻게 설계하느냐**다. 인증은 장애가 나면 사용자 전부가 막힌다는 점에서 SLA에 직접 영향을 주는 영역이고, 보안 이슈(CSRF, 세션 고정, 토큰 탈취)는 단 한 건으로도 큰 사고로 직결된다.

이 문서는 HTTP의 stateless 특성에서 출발해 Cookie → Session → Token 순서로 인증 모델을 쌓고, JSP 레거시와 모바일 API가 공존하는 구체 패턴, CSRF/CORS/SameSite의 실전 함정, 흔한 인증 장애 회복 시나리오를 한 번에 정리한다.

## HTTP는 왜 stateless인가, 그리고 그게 무슨 뜻인가
...
## Token (JWT 포함) — 클라이언트가 상태를 들고 있는 모델
...
## Refresh Token — 짧은 access token과 긴 refresh token의 분리
...
## Stateful vs Stateless — 어디에 둘 것인가
...
## JSP/jQuery 레거시 세션 + 모바일 API 토큰 공존 설계

외식·매장 운영 백엔드는 다음 구성이 흔하다.
...
2. **운영 특성** — 세션은 즉시 로그아웃에 강하지만 외부 세션 스토어가 필요하다. 토큰은 수평 확장과 다채널에 강하지만 즉시 폐기가 어려워 access token을 짧게 가져가고 refresh token을 분리한다.
3. **공존 설계** — JSP 어드민과 모바일 API가 공존하는 상황에서 어드민은 세션, API는 JWT로 분리하고, refresh token은 HttpOnly 쿠키 + rotation 방식으로 운영한다.
4. **trade-off 자각** — 토큰 즉시 무효화가 필요한 케이스(권한 강제 회수 등)에는 서버 측 블랙리스트 또는 짧은 access token + 빠른 refresh로 대응하고, 완전 stateless를 고집하지 않는다.

**CSRF와 CORS는 다른 문제다**

- CSRF는 인증 쿠키 자동 첨부를 악용한 공격, CORS는 브라우저가 크로스 오리진 요청을 합법적으로 허용하기 위한 협상 규약이다.
- CSRF는 세션 기반 인증에서 발생하고, `Authorization` 헤더 기반 API에서는 구조적으로 발생하지 않는다.
- CORS를 푼다고 CSRF 방어가 풀리지 않는다. 둘은 독립이다.

**JWT를 어디에 저장하나**

- access token은 메모리, refresh token은 HttpOnly + Secure + SameSite 쿠키에 둔다.
- localStorage는 XSS 한 방에 털리기 때문에 운영 서비스에서는 피한다.
- 토큰 rotation과 만료 정책을 같이 적용한다.
